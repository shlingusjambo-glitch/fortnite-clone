import { V3, mul, translate, rotY, rotZ, rotX, rand } from './math';
import { MB, Col, rgb, dk, lt, C, LBox, FH, aoY } from './models';
...
class Kit {
  ...
  shelfRack(x: number, y: number, z: number, yaw = 0, w = 3, tiers = 3, stock = true) { this.b.push(mul(translate(x, y, z), rotY(yaw))); for (const sx of [-1, 1]) for (const sz of [-1, 1]) this.b.box([sx * (w / 2 - 0.04), 1.05, sz * 0.45], [0.06, 2.1, 0.06], STEEL); for (let s = 0; s < tiers; s++) { const yy = 0.2 + s * 0.65; this.b.box([0, yy, 0], [w, 0.05, 1.0], rgb(0xcdd3d8)); if (stock) for (let i = 0; i < Math.floor(w / 0.45); i++) if (rand() < 0.75) this.b.rbox([-w / 2 + 0.25 + i * 0.45, yy + 0.22, (rand() - 0.5) * 0.4], [0.32, 0.36, 0.32], [0.3 + rand() * 0.6, 0.3 + rand() * 0.5, 0.3 + rand() * 0.6], 0.03); } this.b.pop(); this.boxes.push({ min: [x - w / 2, y, z - 0.5], max: [x + w / 2, y + 2.1, z + 0.5] }); }
