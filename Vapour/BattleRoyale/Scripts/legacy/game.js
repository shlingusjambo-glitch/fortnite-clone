import { add, sub, scale, norm, len, clamp, lerp, rand, mul, perspective, lookAt, trs, translate, rotY, rotX, rotZ, transformPoint, scaleM } from "./math";
...
const editedMesh = (type, mat, mask) => {
  const k = `${type}_${mat}_${mask}`;
  let m = editCache.get(k);
  if (!m) {
    m = editedPiece(R, type, mat, mask);
    editCache.set(k, m);
  }
  return m;
};
const CHARS = SKINS.map((s) => buildCharacter(R, s));
...
W.drawLabels(H.bigmap.querySelector("canvas"));
{
  ...
  for (let i = 0; i < 60; i++) {
    const a = pts[i], b = pts[(i * 7 + 3) % 60], c = pts[(i * 13 + 5) % 60];
    const l = 35 + rand(0, 35);
...
const baked = [];
{
  ...
  for (const s of W.statics) {
    ...
    if (!cell) {
      cell = { data: [], c: [(Math.floor(s.pos[0] / 48) + 0.5) * 48, 0, (Math.floor(s.pos[2] / 48) + 0.5) * 48] };
      cells.set(key, cell);
    }
...
function propCell(key) {
  ...
  for (const q of W.props) {
    ...
    const src = M[q.type].data, m = trs(q.pos, q.yaw, 0, q.s), c = Math.cos(q.yaw), sn = Math.sin(q.yaw);
    cx = (Math.floor(q.pos[0] / PC) + 0.5) * PC;
    cz = (Math.floor(q.pos[2] / PC) + 0.5) * PC;
    for (let i = 0; i < src.length; i += 9) {
      const p = transformPoint(m, [src[i], src[i + 1], src[i + 2]]), nx = src[i + 3], nz = src[i + 5];
      data.push(p[0], p[1], p[2], nx * c + nz * sn, src[i + 4], -nx * sn + nz * c, src[i + 6], src[i + 7], src[i + 8]);
    }
  }
  cell = data.length ? { mesh: R.upload(new Float32Array(data)), c: [cx, 0, cz] } : null;
  propCells.set(key, cell);
  return cell;
}
let AC = null;
function beep(f, dur, type = "square", vol = 0.08, slide = 0) {
  ...
  g.gain.value = vol;
  g.gain.exponentialRampToValueAtTime(1e-3, AC.currentTime + dur);
...
function botVoice(b) {
  ...
  const a = new Audio("assets/Audio/" + BOT_VOICES[Math.floor(rand(0, BOT_VOICES.length))]);
  a.volume = clamp(1 - len(sub(b.pos, P.pos)) / 65, 0.08, 0.55) * S.master * S.voice;
  if (a.volume > 0.01) a.play().catch(() => {
  });
}
const RARITIES = ["common", "uncommon", "rare", "epic", "legendary"];
const RAR_MULT = [0.85, 0.93, 1, 1.08, 1.16];
const WEAPONS = {
  ar: { name: "Assault Rifle", dmg: 33, rpm: 330, mag: 30, reload: 2.3, spread: 8e-3, pellets: 1, ammo: "medium", auto: true, hs: 1.5, range: 300, rarity: "rare", bloom: 0.012, kick: 0.012 },
  burst: { name: "Burst Assault Rifle", dmg: 33, rpm: 900, mag: 30, reload: 2.6, spread: 6e-3, pellets: 1, ammo: "medium", auto: true, hs: 1.5, range: 300, rarity: "uncommon", burst: 3, bloom: 6e-3, kick: 0.01 },
  smg: { name: "Submachine Gun", dmg: 18, rpm: 720, mag: 30, reload: 2, spread: 0.02, pellets: 1, ammo: "light", auto: true, hs: 1.5, range: 150, rarity: "uncommon", bloom: 0.02, kick: 6e-3 },
  ...
  hunting: { name: "Hunting Rifle", dmg: 86, rpm: 40, mag: 1, reload: 1.9, spread: 2e-3, pellets: 1, ammo: "heavy", auto: false, hs: 2.5, range: 500, rarity: "uncommon", bloom: 0, kick: 0.045 },
  scar: { name: "SCAR", dmg: 36, rpm: 330, mag: 30, reload: 2.1, spread: 6e-3, pellets: 1, ammo: "medium", auto: true, hs: 1.5, range: 320, rarity: "legendary", bloom: 0.01, kick: 0.011 },
  revolver: { name: "Revolver", dmg: 54, rpm: 55, mag: 6, reload: 2.4, spread: 8e-3, pellets: 1, ammo: "medium", auto: false, hs: 2, range: 150, rarity: "uncommon", bloom: 0.012, kick: 0.03 },
  silenced: { name: "Suppressed Pistol", dmg: 26, rpm: 380, mag: 16, reload: 1.4, spread: 0.01, pellets: 1, ammo: "light", auto: false, hs: 2, range: 130, rarity: "epic", bloom: 8e-3, kick: 8e-3 },
...
const CONS = {
  ...
  bushItem: { name: "Bush", dur: 1, rarity: "legendary", use: () => {
    P.bush = true;
    return true;
  } },
...
const S = { ...SDEF, ...lowEnd ? { shadows: 1, grass: 1, scale: 0.8, viewDist: 0 } : {}, ...JSON.parse(localStorage.getItem("fn-settings") || "{}") };
const GALLERY = new URLSearchParams(location.search).get("gallery");
if (GALLERY) {
  document.getElementById("lobby").style.display = "none";
  document.getElementById("lobbybg").style.display = "none";
}
const D = { aimbot: false, esp: false, invuln: false, infMats: false, infAmmo: false, fly: false, lowGrav: false, pauseBots: false };
let lastEmote = 0;
const PROF_DEF = { name: "Player", xp: 0, vbucks: 2765, wins: 0, matches: 0, kills: 0, dmg: 0, builds: 0, chests: 0, discovered: [], unlocked: [0, 1, 2, 3], claimed: [], emote: 0, mode: 0 };
...
function refreshLobby() {
  updateWallet();
  const lv = level(), into = PR.xp % 1e3;
  $("lpanel").querySelector(".lvl").textContent = "LEVEL " + lv;
  $("lpanel").querySelector(".xp i").style.width = into / 10 + "%";
  $("nametag").querySelector("small").textContent = String(lv);
  $("nametag").children[1].innerHTML = `${PR.name}<div class="nr">${PR.matches ? PR.wins + " wins \xB7 " + PR.kills + " elims" : "Not Ready"}</div>`;
  const ms = $("lpanel").querySelectorAll(".mission");
  ...
  const md = $("lpanel").querySelectorAll(".medal");
  md.forEach((m, i) => m.classList.toggle("gold", i < Math.min(10, Math.floor(PR.kills / 5))));
  document.querySelector("#rpanel .solo").textContent = MODES[PR.mode][0];
  document.querySelector("#rpanel .br").textContent = MODES[PR.mode][1];
  $("lbot").firstChild.textContent = "CHAPTER 1 \xB7 SEASON 1";
}
function toast(msg) {
  const e = $("toast");
  e.textContent = msg;
  ...
  clearTimeout(e._t);
  e._t = setTimeout(() => e.style.display = "none", 2200);
}
...
const look = () => [Math.sin(P.yaw) * Math.cos(P.pitch), Math.sin(P.pitch), Math.cos(P.yaw) * Math.cos(P.pitch)];
const curItem = () => P.slot < 0 ? null : P.inv[P.slot];
const items = [], bots = [], fx = [], feed = [];
const chests = [];
const nades = [];
const dying = [];
const pads = [];
const drops = [], meteors = [];
let event = null, eventT = 0;
const bus = { a: [0, 0, 0], b: [0, 0, 0], t: 0, dur: 55, pos: [0, 0, 0], yaw: 0 };
const storm = { c: [0, 0], r: 520, phaseT: 120, phase: 0, shrinking: false, from: { c: [0, 0], r: 380 }, to: { c: [0, 0], r: 380 }, shrinkT: 0 };
const PHASES = [[100, 50, 230], [70, 45, 140], [60, 40, 80], [50, 35, 40], [40, 30, 15], [30, 30, 3]];
function nextStormPhase() {
  for (const b of bots) if (!b.dead) {
    b.skill = Math.min(1, b.skill + 0.06);
    b.accuracy = Math.min(0.75, b.accuracy + 0.03);
    b.reaction = Math.max(0.12, b.reaction - 0.05);
  }
  const ph = PHASES[Math.min(storm.phase, PHASES.length - 1)];
  storm.from = { c: [storm.c[0], storm.c[1]], r: storm.r };
  const a = rand(0, 6.28), d = rand(0, Math.max(0, storm.r - ph[2]) * 0.6);
  storm.to = { c: [storm.c[0] + Math.cos(a) * d, storm.c[1] + Math.sin(a) * d], r: ph[2] };
  const fast = PR.mode === 3 ? 0.45 : 1;
  storm.shrinking = true;
  storm.shrinkT = ph[1] * fast;
  storm.phaseT = ph[1] * fast;
  storm.phase++;
  banner("STORM EYE SHRINKING", "", 4);
}
let siegeUp = false;
let fadeT = 0, fadeDur = 1;
function fade(d) {
  fadeT = d;
  fadeDur = d;
...
function banner(h, p, t2) {
  H.banner.querySelector("h1").textContent = h;
  H.banner.querySelector("p").textContent = p;
  H.banner.querySelector("p").style.display = p ? "block" : "none";
  H.banner.style.display = "block";
  bannerT = t2;
}
...
function startMatch() {
  ...
  P.inv.fill(null);
  P.build = false;
  P.mats = { wood: 0, stone: 0, metal: 30 };
  ...
  bus.a = [Math.cos(a) * 470 + nx * off, alt, Math.sin(a) * 470 + nz * off];
  bus.b = [-Math.cos(a) * 470 + nx * off, alt, -Math.sin(a) * 470 + nz * off];
  bus.t = -1;
  bus.pos = [...bus.a];
  bus.yaw = Math.atan2(bus.b[0] - bus.a[0], bus.b[2] - bus.a[2]);
  P.pos = [ISLAND[0] + rand(-10, 10), 8, ISLAND[2] + rand(-10, 10)];
  P.vel = [0, 0, 0];
  P.yaw = rand(0, 6.28);
  P.pitch = -0.1;
  P.mats = { wood: 999, stone: 999, metal: 999 };
  P.ammo = { light: 999, medium: 999, heavy: 99, shells: 99 };
  P.inv[0] = mkItem("ar", 1, 2);
  P.inv[1] = mkItem("shotgun", 1, 2);
  P.slot = -1;
  for (let i = 0; i < 6; i++) {
    const aa = i / 6 * 6.283;
    dropItem(mkItem(["ar", "shotgun", "sniper", "smg", "pistol", "burst"][i], 1, 2), [ISLAND[0] + Math.cos(aa) * 12, 6, ISLAND[2] + Math.sin(aa) * 12]);
  }
  storm.c = [rand(-80, 80), rand(-80, 80)];
  storm.r = 520;
  ...
  for (let i = 0; i < 32; i++) {
    const b = spawnBot(void 0, PR.mode === 1 || i < 15 ? i % 2 ? 4 : 5 : Math.floor(rand(0, 4)));
    b.joinT = rand(1, 18);
    b.state = "island";
    b.pos = [ISLAND[0] + rand(-25, 25), 8, ISLAND[2] + rand(-25, 25)];
    const ab = sub(bus.b, bus.a), k = clamp(((b.land[0] - bus.a[0]) * ab[0] + (b.land[2] - bus.a[2]) * ab[2]) / (ab[0] * ab[0] + ab[2] * ab[2]), 0.08, 0.95);
  ...
  H.end.style.display = "none";
  H.lobby.style.display = "none";
  H.hud.style.display = "block";
  try {
...
function spawnBot(at, profileIdx = -1) {
  const p = POIS[Math.floor(rand(0, POIS.length))], land = [p.x + rand(-p.r, p.r) * 0.8, 0, p.z + rand(-p.r, p.r) * 0.8];
...
addEventListener("gamepadconnected", (e) => {
  ...
  info("\u{1F3AE} CONTROLLER CONNECTED");
  rumble(160, 0.4, 0.6);
});
addEventListener("gamepaddisconnected", (e) => {
  if (gpIndex === e.gamepad.index) {
    gpIndex = null;
    info("\u{1F3AE} CONTROLLER DISCONNECTED");
  }
});
addEventListener("keydown", (e) => {
  if (!keys.has(e.code)) pressed.add(e.code);
  keys.add(e.code);
  if (e.code === "Tab" || e.code.startsWith("F") || e.code.startsWith("Alt")) e.preventDefault();
});
addEventListener("keyup", (e) => keys.delete(e.code));
addEventListener("blur", () => keys.clear());
canvas.addEventListener("mousedown", (e) => {
  if (P.state === "lobby") return;
...
addEventListener("wheel", (e) => {
  ...
  let s = P.slot;
  for (let i = 0; i < n + 1; i++) {
    s = (s + 1 + (e.deltaY > 0 ? 1 : -1) + (n + 1) * 2) % (n + 1) - 1;
    if (s < 0 || P.inv[s]) break;
  }
  P.slot = s;
});
$("btnPlay").onclick = () => {
...
const SHOP = [4, 5, 6, 7, 8, 9, 10, 11].map((i, k) => ({ skin: i, price: [800, 1200, 1500, 2e3][k % 4] }));
const PASS = Array.from({ length: 20 }, (_, i) => ({ tier: i + 1, reward: i % 5 === 4 ? { skin: 12 + Math.floor(i / 5) } : { vbucks: 100 + i % 5 * 50 } }));
const CHALLENGES = [["Eliminate 10 opponents", () => PR.kills, 10, 500], ["Deal 2,500 damage", () => Math.round(PR.dmg), 2500, 500], ["Place 250 structures", () => PR.builds, 250, 400], ["Open 25 chests", () => PR.chests, 25, 300], ["Discover every named location", () => PR.discovered.length, POIS.length, 800], ["Win a Victory Royale", () => PR.wins, 1, 1500], ["Play 10 matches", () => PR.matches, 10, 300]];
const skinTile = (i, sub2, cls = "") => `<div class="tile skin ${cls}" data-skin="${i}"><canvas width="64" height="64"></canvas><b>${SKINS[i].name}</b>${sub2}</div>`;
function paintSkins() {
  menuCards.querySelectorAll(".tile.skin canvas").forEach((cv) => {
    const sk = SKINS[+cv.parentElement.dataset.skin], c = cv.getContext("2d"), col = (v) => `rgb(${v.map((x) => x * 255 | 0).join(",")})`;
    c.fillStyle = col(sk.top);
    c.fillRect(12, 34, 40, 30);
    c.fillStyle = col(sk.pants);
    c.fillRect(16, 56, 32, 8);
    c.fillStyle = col(sk.skin);
    c.fillRect(18, 10, 28, 26);
    c.fillStyle = col(sk.hair);
    c.fillRect(16, 4, 32, 10);
    c.fillStyle = "#000";
    c.fillRect(24, 20, 4, 4);
...
function openPage(name) {
  ...
  if (name === "BATTLE PASS") {
    const lv = level();
    html = `<div class="tile"><b>LEVEL ${lv}</b>${PR.xp % 1e3} / 1000 XP to next level \xB7 earn XP from eliminations, damage and survival</div>` + PASS.map((t2) => {
      const ok = lv >= t2.tier, done = PR.claimed.includes(t2.tier);
      const r = "skin" in t2.reward ? SKINS[t2.reward.skin].name : `\u24CB ${t2.reward.vbucks}`;
      return `<div class="tile ${ok ? "" : "locked"}"><b>TIER ${t2.tier}</b>${r}<br>${done ? "\u2714 CLAIMED" : ok ? `<button data-claim="${t2.tier}">CLAIM</button>` : `Reach level ${t2.tier}`}</div>`;
    }).join("");
  } else if (name === "CHALLENGES") html = CHALLENGES.map(([n, f, goal, xp], i) => {
    const v = Math.min(goal, f()), done = v >= goal;
    return `<div class="tile"><b>${n}</b><div class="bar"><i style="width:${v / goal * 100}%"></i><b>${v} / ${goal}</b></div>${done ? "\u2714 +" + xp + " XP" : "+" + xp + " XP"}</div>`;
  }).join("");
  ...
  else if (name === "LOCKER") html = `<div class="tile" style="grid-column:1/-1"><b>EMOTE (B)</b>${EMOTES.map((e, i) => `<button data-emote="${i}" ${PR.emote === i ? 'style="background:#ffe22e;color:#12305a"' : ""}>${e}</button>`).join(" ")}</div>` + SKINS.map((sk, i) => skinTile(i, PR.unlocked.includes(i) ? i === P.skin ? "EQUIPPED" : "Click to equip" : "\u{1F512} Item Shop / Battle Pass", i === P.skin ? "on" : PR.unlocked.includes(i) ? "" : "locked")).join("");
  ...
  menuCards.querySelectorAll(".tile.skin").forEach((card) => card.onclick = () => {
    const i = +card.dataset.skin;
    if (name === "LOCKER" && PR.unlocked.includes(i)) {
      P.skin = i;
      PR.mode = PR.mode;
      saveProfile();
      openPage(name);
  ...
  menuCards.querySelectorAll("[data-buy]").forEach((b) => b.onclick = (e) => {
    e.stopPropagation();
    const i = +b.dataset.buy, pr = +b.dataset.price;
    if (PR.vbucks < pr) return toast("Not enough V-Bucks");
    PR.vbucks -= pr;
    PR.unlocked.push(i);
    updateWallet();
    toast("Purchased " + SKINS[i].name);
    openPage(name);
  });
  menuCards.querySelectorAll("[data-claim]").forEach((b) => b.onclick = () => {
    const t2 = PASS[+b.dataset.claim - 1];
    PR.claimed.push(t2.tier);
    if ("skin" in t2.reward) {
      PR.unlocked.push(t2.reward.skin);
      toast("Unlocked " + SKINS[t2.reward.skin].name);
    } else {
      PR.vbucks += t2.reward.vbucks;
      toast("+" + t2.reward.vbucks + " V-Bucks");
    }
    updateWallet();
    openPage(name);
  });
  menuCards.querySelectorAll("[data-vb]").forEach((b) => b.onclick = () => {
    PR.vbucks += +b.dataset.vb;
    updateWallet();
    toast("+" + b.dataset.vb + " V-Bucks");
  });
  menuCards.querySelectorAll("[data-mode]").forEach((b) => b.onclick = () => {
    PR.mode = +b.dataset.mode;
    refreshLobby();
    menuPage.style.display = "none";
    $("btnPlay").click();
  });
  menuCards.querySelectorAll("[data-emote]").forEach((b) => b.onclick = () => {
    PR.emote = +b.dataset.emote;
    lastEmote = PR.emote;
    saveProfile();
    openPage(name);
  });
  menuCards.querySelectorAll("[data-act=reset]").forEach((b) => b.onclick = () => {
    if (confirm("Wipe the local profile?")) {
      localStorage.removeItem("fn-profile");
      location.reload();
    }
  });
}
document.querySelectorAll("#lnav .tab").forEach((el) => el.onclick = () => openPage(el.textContent || ""));
$("menuClose").onclick = () => openPage("PLAY");
document.querySelector("#lnav .box").onclick = () => openPage("PARTY");
document.querySelectorAll("#lobby .plus").forEach((el) => el.onclick = () => openPage("PARTY"));
$("local").onclick = () => toast("LOCAL: everything runs in this browser. No account, no servers.");
$("nametag").onclick = () => {
  const n = prompt("Display name", PR.name);
  if (n && n.trim()) {
    PR.name = n.trim().slice(0, 16);
    saveProfile();
    refreshLobby();
  }
};
$("lbot").querySelector("span").onclick = (e) => {
  const tx = e.target.textContent || "";
  if (tx.includes("Controls")) {
    settingsOpen(true);
    SET.querySelector("[data-p=keys]").click();
  } else openPage("LOCKER");
};
...
function botBoxes() {
  const b = [];
  for (const d of bots) if (!d.dead) {
    const [x, y, z] = d.pos;
    b.push({ min: [x - 0.35, y, z - 0.25], max: [x + 0.35, y + 1.55, z + 0.25], ref: { d, head: false } }, { min: [x - 0.25, y + 1.55, z - 0.25], max: [x + 0.25, y + 2.05, z + 0.25], ref: { d, head: true } });
  }
  return b;
}
function damage(n, by = "the storm") {
  ...
  P.bush = false;
  const s = Math.min(P.shield, n);
  P.shield -= s;
  P.hp -= n - s;
  H.flash.style.opacity = "0.3";
  setTimeout(() => H.flash.style.opacity = "0", 80);
  beep(120, 0.2, "sawtooth", 0.1, -60);
  if (P.hp <= 0) {
    P.hp = 0;
    P.dead = true;
    P.dyingT = 1.5;
...
function endScreen(win, by = "") {
  ...
  if (win) {
    const c = H.end.querySelector(".confetti");
    c.innerHTML = "";
...
let infoT = 0;
function info(t2) {
  H.info.textContent = t2;
  H.info.style.display = "block";
  infoT = 2;
}
...
const camPos = [0, 0, 0];
let camFwd = [0, 0, 1], fov = 1.15;
let VP = perspective(1, 1, 0.1, 10);
function project(p) {
  const c = transformPoint(VP, p);
  const w = VP[3] * p[0] + VP[7] * p[1] + VP[11] * p[2] + VP[15];
  if (w < 0.1 || Math.abs(c[0]) > 1.2 || Math.abs(c[1]) > 1.2) return null;
  return [(c[0] * 0.5 + 0.5) * innerWidth, (0.5 - c[1] * 0.5) * innerHeight];
}
function buildTarget() {
  const dir = (Math.round(P.yaw / (Math.PI / 2)) % 4 + 4) % 4, a = dir * Math.PI / 2, f = [Math.sin(a), 0, Math.cos(a)];
  let level2 = Math.floor((P.pos[1] + 1) / 4) * 4;
  if (P.pitch > 0.45) level2 += 4;
  let t2 = add(P.pos, scale(f, P.piece === "wall" ? 2.6 : 3.2));
  if (P.pitch < -0.7 && P.piece !== "wall") t2 = P.pos;
  const cx = Math.floor(t2[0] / 4) * 4 + 2, cz = Math.floor(t2[2] / 4) * 4 + 2;
  if (P.piece === "wall") return { type: "wall", pos: [cx + f[0] * 2, level2, cz + f[2] * 2], dir };
  return { type: P.piece, pos: [cx, level2, cz], dir: P.piece === "ramp" ? (dir + P.rampRot) % 4 : dir };
}
function botDamage(dm, n, by, how = "with a weapon") {
  ...
  if (att) {
    dm.memory = [...att];
    dm.memoryT = t;
    if (Math.random() < 0.4 + dm.skill * 0.6) {
      dm.yaw = Math.atan2(att[0] - dm.pos[0], att[2] - dm.pos[2]);
      dm.retarget = 0;
    }
    if (!dm.enemy && dm.weapon && dm.mode !== "heal") dm.mode = "hunt";
  }
  if (dm.hp > 0) {
    if (Math.random() < 0.22) botVoice(dm);
    return;
  }
  dm.dead = true;
  P.alive--;
  dying.push({ skin: dm.skin, pos: [...dm.pos], yaw: dm.yaw, t: 1.4 });
  fx.push({ kind: "puff", t: 0.4, pos: add(dm.pos, [0, 1, 0]), col: [0.4, 0.8, 1] });
  if (Math.random() < 0.35) {
    const killer = bots.find((x) => x.name === by);
  ...
  if (by === "Player") {
    ...
    setTimeout(() => H.elim.style.display = "none", 2500);
    addFeed(`Player eliminated <span class="v">${dm.name}</span> ${how}`);
    beep(600, 0.3, "square", 0.08, 300);
  } else addFeed(`${by} eliminated <span class="v">${dm.name}</span>`);
  for (const w of dm.weapons) dropItem(mkItem(w), add(dm.pos, [0, 0.2, 0]), 1.2);
  dropItem(mkItem("bandage", 3), add(dm.pos, [0, 0.2, 0]), 1);
  if (dm.heals > 1) dropItem(mkItem("shieldPot", 1), add(dm.pos, [0, 0.2, 0]), 1.3);
...
function shoot(item) {
  ...
  if (!D.infAmmo) item.mag--;
  P.fireCd = 60 / w.rpm;
  beep(item.kind === "sniper" ? 90 : item.kind === "shotgun" ? 110 : item.kind === "silenced" ? 500 : 220, 0.12, "sawtooth", item.kind === "silenced" ? 0.04 : 0.12, -80);
  ...
  const rStrong = item.kind === "shotgun" ? 0.9 : item.kind === "sniper" ? 1 : item.kind === "smg" ? 0.3 : 0.5;
  rumble(rDur, rWeak, rStrong);
  P.pitch += w.kick * (P.ads ? 0.6 : 1);
  P.yaw += rand(-w.kick, w.kick) * 0.4;
  if (w.burst) {
    if (P.burstLeft <= 0) P.burstLeft = w.burst;
    P.burstLeft--;
    if (P.burstLeft <= 0) P.fireCd = 0.5;
  }
  ...
  for (let i = 0; i < w.pellets; i++) {
    ...
    if (D.aimbot) {
      ...
      for (const b of bots) if (!b.dead && b.state === "ground") {
        const hp = add(b.pos, [0, 1.75, 0]), dd = len(sub(hp, camPos));
        if (dd < best && dd < w.range) {
          best = dd;
          bp = hp;
        }
      }
      if (bp) aim = norm(sub(bp, camPos));
    }
  ...
  if (hitAny) {
    ...
    H.hitm.className = headAny ? "head" : "";
    setTimeout(() => H.hitm.style.opacity = "0", 60);
    beep(headAny ? 1400 : 1e3, 0.06, "sine", 0.1);
  }
}
function swingPickaxe() {
  ...
  if (!h) return;
  const weak = !!(P.weakRef === h.ref && P.weakPos && len(sub(h.p, P.weakPos)) < 0.9), mark = () => {
    P.weakRef = h.ref;
    P.weakPos = add(h.p, [rand(-0.45, 0.45), rand(-0.45, 0.45), rand(-0.08, 0.08)]);
    P.weakT = 4;
  };
  if (weak) rumble(120, 0.8, 0.85);
  else rumble(75, 0.45, 0.45);
  if (h.kind === "prop") {
    const q = h.ref, dmg = weak ? 100 : 50;
    q.hp -= dmg;
    const m = q.type === "rock" ? "stone" : "wood";
    const n = q.type === "bush" ? 3 : weak ? 24 : 10;
    giveMat(m, n);
    fx.push({ kind: "dmg", t: 0.7, pos: h.p, text: weak ? "CRITICAL +" + n : "+" + n, head: weak });
    beep(weak ? 950 : 500, 0.1, "square", 0.06);
    if (q.hp <= 0) {
      q.dead = 30;
      P.weakT = 0;
      dirtyProp(q);
    ...
    const s = h.ref;
    giveMat(s.mesh === "crate" || s.mesh === "fence" || s.mesh === "bench" ? "wood" : s.mesh === "hedge" ? "wood" : "metal", 5);
    fx.push({ kind: "dmg", t: 0.7, pos: h.p, text: "+5" });
    beep(430, 0.1, "square", 0.06);
  } else if (h.kind === "static") {
    const s = h.ref, dmg = weak ? 100 : 45, mat = s.mesh === "car" || s.mesh === "truck" || s.mesh === "lamp" ? "metal" : s.mesh.startsWith("house") ? "wood" : "stone", n = weak ? 18 : 7;
    s.hp = (s.hp ?? 300) - dmg;
    s.shake = 0.28;
    giveMat(mat, n);
    fx.push({ kind: "dmg", t: 0.7, pos: h.p, text: weak ? "CRITICAL +" + n : "+" + n, head: weak });
    beep(weak ? 900 : 430, 0.1, "square", 0.06);
    if (s.hp <= 0) {
      s.dead = true;
      s.boxes.length = 0;
      P.weakT = 0;
    ...
    W.damagePiece(p, 50);
    giveMat(p.mat, 5);
    fx.push({ kind: "dmg", t: 0.7, pos: h.p, text: "50" });
    beep(400, 0.1, "square", 0.06);
  } else if (h.kind === "box") {
    const dm = h.ref.d;
    botDamage(dm, 20, "Player", "with a pickaxe");
    P.dmg += 20;
    fx.push({ kind: "dmg", t: 0.7, pos: h.p, text: "20" });
  }
}
function moveEntity(e, h, dt) {
  const hw = 0.35, travel = Math.max(Math.abs(e.vel[0]), Math.abs(e.vel[1]), Math.abs(e.vel[2])) * dt, steps = Math.max(1, Math.ceil(travel / 0.16)), sdt = dt / steps;
  let landedNow = false;
  for (let step = 0; step < steps; step++) {
    const boxes = W.solids(e.pos[0], e.pos[2]);
    const overlaps = (p) => boxes.filter((b) => p[0] + hw > b.min[0] && p[0] - hw < b.max[0] && p[1] < b.max[1] && p[1] + h > b.min[1] && p[2] + hw > b.min[2] && p[2] - hw < b.max[2]);
    for (const ax of [0, 2, 1]) {
      const d = e.vel[ax] * sdt;
      if (!d) continue;
      e.pos[ax] += d;
      let ov = overlaps(e.pos);
      if (ov.length && ax !== 1) {
        const up = [e.pos[0], e.pos[1] + 0.7, e.pos[2]];
        if (!overlaps(up).length) {
          e.pos[1] += 0.7;
          ov = [];
        }
      }
      for (const b of ov) {
        if (d > 0) e.pos[ax] = b.min[ax] - (ax === 1 ? h : hw) - 1e-3;
        else e.pos[ax] = b.max[ax] + (ax === 1 ? 0 : hw) + 1e-3;
        if (ax === 1) {
          if (d < 0) {
            landedNow = true;
            e.grounded = true;
  ...
  if (e.pos[1] <= g + 0.01 && e.vel[1] <= 0) {
    e.pos[1] = g;
    if (!e.grounded) landedNow = true;
    e.grounded = true;
    e.vel[1] = 0;
  }
  if (e.pos[1] < -1.6) {
...
function moveAndCollide(dt) {
  if (moveEntity(P, height(), dt)) landed();
}
function landed() {
  if (P.state === "glide" || P.state === "sky") {
    P.state = "play";
    if (PR.mode === 2 && !siegeUp) {
      siegeUp = true;
      const c = fortAt([storm.c[0], terrainH(storm.c[0], storm.c[1]), storm.c[1]], "stone", 3);
      for (let i = 0; i < 6; i++) {
        const b = spawnBot([c[0] + rand(-3, 3), c[1] + 4.5, c[2] + rand(-3, 3)], 5);
        b.name = "Defender" + (i + 1);
        b.weapon = i % 2 ? "ar" : "shotgun";
        b.weapons = [b.weapon];
        b.ammo = { light: 90, medium: 120, heavy: 10, shells: 30 };
        b.mats = 400;
      }
      banner("FORTRESS SIEGE", "A STONE FORT HOLDS THE CIRCLE CENTRE \xB7 TAKE IT", 5);
    }
    return;
  }
  if (P.vel[1] < -22) {
    const d = Math.round((-P.vel[1] - 22) * 4);
    damage(d);
    info(`Fall damage -${d}`);
  }
}
const plIcon = H.pl.querySelector("canvas");
function drawIcon(skin) {
  const c = plIcon.getContext("2d");
  const col = (v) => `rgb(${v.map((x) => x * 255 | 0).join(",")})`;
  c.clearRect(0, 0, 16, 16);
  c.fillStyle = col(skin.top);
  c.fillRect(3, 11, 10, 5);
  c.fillStyle = col(skin.skin);
  c.fillRect(4, 3, 8, 8);
  c.fillStyle = col(skin.hair);
  c.fillRect(3, 1, 10, 3);
  c.fillStyle = "#000";
  c.fillRect(6, 6, 1, 1);
  c.fillRect(10, 6, 1, 1);
}
const mmCtx = H.mm.querySelectorAll("canvas")[1].getContext("2d");
const mmBg = H.mm.querySelectorAll("canvas")[0].getContext("2d");
const HEAD = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"];
const fmt = (s) => `${Math.floor(s / 60)}:${String(Math.floor(s % 60)).padStart(2, "0")}`;
let fpsN = 0, fpsT = 0, fpsV = 0, lowT = 0;
let hudN = 0;
function drawHud() {
  const heavy = ++hudN % 3 === 0;
  H.hp.querySelector("i").style.width = P.hp + "%";
  H.hp.nextElementSibling.textContent = String(Math.ceil(P.hp));
  H.sh.querySelector("i").style.width = P.shield + "%";
  H.sh.nextElementSibling.textContent = String(Math.ceil(P.shield));
  H.pl.querySelector(".b i").style.width = P.hp + "%";
  if (heavy) H.mats.innerHTML = ["wood", "stone", "metal"].map((m) => `<div class="${P.mat === m && P.build ? "sel" : ""}">${m === "wood" ? '<svg viewBox="0 0 40 40"><path d="M6 30 L26 8 L34 14 L14 36 Z" fill="#e6c48a" stroke="#8a6a3a" stroke-width="1.5"/></svg>' : m === "stone" ? '<svg viewBox="0 0 40 40"><path d="M4 22 L20 12 L36 20 L20 30 Z" fill="#c9c9c9" stroke="#666" stroke-width="1.5"/><path d="M4 22 L20 30 L20 36 L4 28 Z" fill="#a0a0a0" stroke="#666" stroke-width="1.5"/><path d="M36 20 L20 30 L20 36 L36 26 Z" fill="#8a8a8a" stroke="#666" stroke-width="1.5"/></svg>' : '<svg viewBox="0 0 40 40"><path d="M8 8 L30 8 L30 14 L18 14 L32 32 L10 32 L10 26 L22 26 Z" fill="#dfe6ee" stroke="#556" stroke-width="1.5"/></svg>'}${P.mats[m]}</div>`).join("");
  ...
  H.cross.className = P.build ? "build" : "";
  H.cross.style.display = (P.state === "play" || P.state === "island") && !P.scoped ? "block" : "none";
  H.scope.style.display = P.scoped && !P.over ? "block" : "none";
  H.hud.style.opacity = P.over ? "0" : "1";
  H.prog.style.display = P.useT > 0 ? "block" : "none";
  if (P.useT > 0) H.prog.querySelector("i").style.width = 100 - P.useT / P.useDur * 100 + "%";
  const deg = ((-(P.yaw * 180 / Math.PI) + 180) % 360 + 360) % 360;
  let ch = `<div class="hd">${Math.round(deg)}</div>`;
  for (let d = -90; d <= 90; d += 15) {
    const a = ((Math.round(deg / 15) * 15 + d) % 360 + 360) % 360, x = 410 + (a - deg + 540) % 360 - 180;
    const px = 410 + ((a - deg + 540) % 360 - 180) * 4.2;
  ...
  if (heavy || hudN % 3 === 1) {
    mmBg.clearRect(0, 0, 300, 300);
    mmBg.fillStyle = "#7bbde9";
    mmBg.fillRect(0, 0, 300, 300);
    if (P.state === "island") {
      mmBg.fillStyle = "#8fd44e";
      mmBg.beginPath();
      mmBg.arc(150 + (ISLAND[0] - P.pos[0]) * zoom * 600 / SIZE, 150 + (ISLAND[2] - P.pos[2]) * zoom * 600 / SIZE, 62 * zoom * 600 / SIZE, 0, 6.28);
      mmBg.fill();
    } else mmBg.drawImage(mapCv, sx - vw / 2, sz - vw / 2, vw, vw, 0, 0, 300, 300);
    const g = mmCtx;
    g.clearRect(0, 0, 300, 300);
    const toMM = (x, z) => [150 + ((x + SIZE / 2) / SIZE * 600 - sx) * zoom, 150 + ((z + SIZE / 2) / SIZE * 600 - sz) * zoom];
    g.setLineDash([6, 6]);
    g.strokeStyle = "#fff";
    g.lineWidth = 2;
    g.beginPath();
    g.moveTo(...toMM(bus.a[0], bus.a[2]));
    g.lineTo(...toMM(bus.b[0], bus.b[2]));
    g.stroke();
    g.setLineDash([]);
    const sc = toMM(storm.c[0], storm.c[1]), sr = storm.r / SIZE * 600 * zoom;
    if (P.state !== "island") {
      g.fillStyle = "rgba(150,80,200,0.45)";
      g.fillRect(0, 0, 300, 300);
      g.globalCompositeOperation = "destination-out";
      g.beginPath();
      g.arc(sc[0], sc[1], sr, 0, 6.28);
      g.fill();
    ...
    if (P.state === "bus") {
      const [bx, bz] = toMM(bus.pos[0], bus.pos[2]);
      g.fillStyle = "#4fa8ff";
      g.strokeStyle = "#fff";
      g.beginPath();
      g.rect(bx - 9, bz - 6, 18, 12);
      g.fill();
      g.stroke();
    }
    g.save();
  ...
  let poi = "";
  for (const p of POIS) if (Math.hypot(P.pos[0] - p.x, P.pos[2] - p.z) < p.r + 20) poi = p.name;
  H.mm.querySelector(".poi").textContent = poi;
  if (poi && P.state === "play" && !PR.discovered.includes(poi)) {
    PR.discovered.push(poi);
    banner(poi, "DISCOVERED \xB7 +100 XP", 3);
    PR.xp += 100;
    saveProfile();
  }
  H.stats.innerHTML = `<span>\u{1F552} ${fmt(storm.phaseT)}</span><span>\u{1F464} ${P.alive}</span><span>\u2694 ${P.kills}</span>`;
  ...
  if (D.esp && !S.streamer) {
    for (const d of bots) if (!d.dead && d.state !== "bus") {
      ...
      if (s) html += `<div class="nm" style="left:${s[0]}px;top:${s[1]}px;color:#ff8">${d.name} \xB7 ${Math.ceil(d.hp + d.shield)} \xB7 ${Math.round(len(sub(d.pos, P.pos)))}m</div>`;
    }
  }
  const th = P.state === "play" ? W.raycast(camPos, camFwd, 200, botBoxes()) : null;
  if (th && th.kind === "box") {
    const dm = th.ref.d;
    H.tgt.textContent = `${dm.name} \xB7 ${Math.ceil(dm.hp + dm.shield)} HP \xB7 ${Math.round(th.t)}m`;
    H.tgt.style.display = "block";
  } else if (th && th.kind === "piece" && th.t < 12) {
    const pc = th.ref;
    H.tgt.innerHTML = P.editing ? "LMB select tiles \xB7 X / RMB confirm \xB7 R reset" : `<i style="display:inline-block;width:80px;height:6px;background:#0008;vertical-align:middle;margin-right:8px"><i style="display:block;height:100%;width:${pc.hp / pc.maxHp * 100}%;background:#7cf23a"></i></i>${Math.ceil(pc.hp)} / ${pc.maxHp} \xB7 X to edit`;
    H.tgt.style.display = "block";
  } else H.tgt.style.display = "none";
  const cs = 52 + P.bloom * 2600 * (P.ads ? 0.5 : 1);
  H.cross.style.width = H.cross.style.height = cs + "px";
  H.cross.style.margin = -cs / 2 + "px";
  for (const f of fx) if (f.kind === "dmg") {
    const s = project(add(f.pos, [0, (0.9 - f.t) * 1.5, 0]));
    if (s) html += `<div class="dmg ${f.head ? "head" : ""}" style="left:${s[0]}px;top:${s[1]}px;opacity:${Math.min(1, f.t * 3)}">${f.text}</div>`;
  }
  H.fx.innerHTML = html;
  const ws = P.weakT > 0 && P.weakPos ? project(P.weakPos) : null;
  H.weak.style.display = ws ? "block" : "none";
  if (ws) {
    H.weak.style.left = ws[0] + "px";
    H.weak.style.top = ws[1] + "px";
  }
}
function drawChar(ch, root, a) {
  ...
  if (!a.grounded && a.pose !== "sky" && a.pose !== "glide") {
    thL = -0.6;
    thR = 0.25;
    shL = 1.3;
    shR = 1;
  ...
  if (a.pose === "pick") {
    const sw = a.swing && a.swing > 0 ? Math.sin(a.swing * 6.3) : 0;
    uR = -1.3 - sw * 1.6;
    fR = -0.8 + sw * 0.5;
    zR = 0.05;
    uL = -0.4;
  ...
  if (a.pose === "emote") {
    ...
    if (e === 0) {
      uL = -1.6 + Math.sin(w) * 0.8;
      uR = -1.6 - Math.sin(w) * 0.8;
      zL = 0.9;
      zR = -0.9;
      fL = -1.2;
      fR = -1.2;
      thL = -0.2 + Math.sin(w) * 0.3;
      thR = -0.2 - Math.sin(w) * 0.3;
      shL = shR = 0.5;
      bob = Math.abs(Math.sin(w)) * 0.12;
      yawWig = Math.sin(w * 0.5) * 0.25;
    } else if (e === 1) {
      uR = -2.6;
      fR = -0.6 + Math.sin(w * 1.3) * 0.5;
      zR = -0.4;
      uL = 0.1;
      fL = -0.3;
      thL = thR = shL = shR = 0;
    } else if (e === 2) {
      const f = Math.sin(w * 1.4);
      uL = -0.9;
      uR = -0.9;
      fL = -0.9;
      fR = -0.9;
  ...
  const hip = mul(m, translate(0, 1, 0));
  const upper = mul(mul(hip, rotX(lean)), rotY(twist));
  R.draw(ch.torso, upper, [1, 1, 1], 1, st);
  R.draw(ch.head, mul(mul(upper, translate(0, 0.66, 0)), rotX(-a.pitch * 0.5 - lean * 0.7)), [1, 1, 1], 1, st);
  const armM = (side, u, z, f) => {
    const sh = mul(mul(mul(upper, translate(side * 0.27, 0.5, 0)), rotZ(-side * z)), rotX(u));
    R.draw(ch.upperArm, sh, [1, 1, 1], 1, st);
    const el = mul(mul(sh, translate(0, -0.31, 0)), rotX(f));
    R.draw(ch.foreArm, el, [1, 1, 1], 1, st);
    return mul(el, translate(0, -0.38, 0));
  };
  const handR = armM(-1, uR, zR, fR);
  armM(1, uL, zL, fL);
  const legM = (side, th, sh) => {
    const h = mul(mul(hip, translate(side * 0.12, 0, 0)), rotX(th));
    R.draw(ch.thigh, h, [1, 1, 1], 1, st);
    R.draw(ch.shin, mul(mul(h, translate(0, -0.46, 0)), rotX(sh)), [1, 1, 1], 1, st);
  };
  legM(1, thL, shL);
  legM(-1, thR, shR);
  if (a.held === "pickaxe") R.draw(M.pickaxe, mul(handR, mul(translate(0, 0.05, 0.04), rotX(1.4))));
  else if (a.held) {
    const aiming = a.pose === "aim" || a.pose === "crouch";
    const gm = aiming ? mul(mul(upper, translate(-0.2, 0.36, 0.34)), mul(rotY(-0.08), rotX(-a.pitch * 0.6))) : mul(mul(upper, translate(-0.28, -0.02, 0.2)), mul(rotY(0.45), rotX(-0.95)));
...
function settingsOpen(on) {
  SET.style.display = on ? "block" : "none";
  if (on) {
    document.exitPointerLock();
    syncSettingsUI();
  } else if (P.state !== "lobby" && !P.over) canvas.requestPointerLock();
  H.pause.style.display = "none";
}
function syncSettingsUI() {
  SET.querySelectorAll("[data-s]").forEach((el) => {
    const k = el.dataset.s, v = S[k];
    if (el instanceof HTMLInputElement && el.type === "checkbox") el.checked = !!v;
    else el.value = String(v);
    const val = el.parentElement?.querySelector(".val");
    if (val) val.textContent = typeof v === "number" ? v % 1 ? v.toFixed(2) : String(v) : "";
  });
}
SET.querySelectorAll("[data-s]").forEach((el) => el.oninput = () => {
  ...
  const val = el.parentElement?.querySelector(".val");
  if (val) val.textContent = String(S[k]);
});
SET.querySelectorAll(".tabs div").forEach((tb) => tb.onclick = () => {
  SET.querySelectorAll(".tabs div").forEach((x) => x.classList.toggle("on", x === tb));
  SET.querySelectorAll(".page").forEach((pg) => pg.classList.toggle("on", pg.dataset.p === tb.dataset.p));
});
$("setApply").onclick = () => {
  localStorage.setItem("fn-settings", JSON.stringify(S));
  settingsOpen(false);
  info("Settings saved");
};
$("setReset").onclick = () => {
  Object.assign(S, SDEF);
  syncSettingsUI();
};
...
function emoteJingle(i) {
  const notes = [[440, 554, 659, 880], [523, 659], [392, 494, 587, 494], [330, 262]][i];
  notes.forEach((f, n) => setTimeout(() => beep(f, 0.18, "triangle", 0.06), n * 160));
}
const dbgOpen = () => H.dbg.style.display === "block";
function toggleDbg(on = !dbgOpen()) {
  H.dbg.style.display = on ? "block" : "none";
  if (on) document.exitPointerLock();
  else if (P.state !== "lobby") canvas.requestPointerLock();
  H.pause.style.display = "none";
}
...
function fortAt(c, mat, size = 3) {
  ...
  for (let lvl = 0; lvl < h; lvl++) for (let i = -1; i <= 1; i++) {
    const y = base + lvl * 4;
    W.place("wall", mat, [cx + i * 4, y, cz - 6], 0);
    W.place("wall", mat, [cx + i * 4, y, cz + 6], 0);
    W.place("wall", mat, [cx - 6, y, cz + i * 4], 1);
    W.place("wall", mat, [cx + 6, y, cz + i * 4], 1);
    if (lvl === 1) W.place("floor", mat, [cx + i * 4, y, cz], 0), W.place("floor", mat, [cx + i * 4, y, cz - 4], 0), W.place("floor", mat, [cx + i * 4, y, cz + 4], 0);
  }
  for (const k of [World.key("wall", [cx, base, cz + 6], 0), World.key("floor", [cx, base + 4, cz], 0)]) {
    const p = W.pieces.get(k);
    if (p) W.removePiece(p);
  }
  W.place("ramp", mat, [cx, base, cz], 0);
  return [cx, base, cz];
}
function dbgAction(a) {
  const ahead = add(P.pos, scale(fwd(), 24));
  ahead[1] = terrainH(ahead[0], ahead[2]);
  switch (a) {
    case "sethp":
      P.hp = clamp(+$("dHp").value, 1, 100);
      P.shield = clamp(+$("dSh").value, 0, 100);
      break;
    case "refill":
      P.mats = { wood: 999, stone: 999, metal: 999 };
      P.ammo = { light: 999, medium: 999, heavy: 999, shells: 999 };
      break;
    case "loadout":
      P.inv = [mkItem("tac", 1, 4), mkItem("scar", 1, 4), mkItem("hunting", 1, 4), mkItem("chug", 2), mkItem("miniShield", 6)];
      P.slot = 0;
      P.ammo = { light: 999, medium: 999, heavy: 999, shells: 999 };
      P.mats = { wood: 999, stone: 999, metal: 999 };
      break;
    case "give": {
      const k = $("dItem").value, r = $("dRar").selectedIndex;
      let sl = P.inv.indexOf(null);
      if (sl < 0) sl = Math.max(0, P.slot);
      P.inv[sl] = mkItem(k, isWeapon(k) ? 1 : 3, r);
      P.slot = sl;
      if (isWeapon(k)) P.ammo[WEAPONS[k].ammo] += 90;
      break;
    }
    case "tp": {
      const p = POIS[+$("dPoi").value];
      P.pos = [p.x, terrainH(p.x, p.z) + 2, p.z];
      P.vel = [0, 0, 0];
      if (P.state !== "play") P.state = "play";
      break;
    }
    case "bus":
      startMatch();
    ...
    case "storm":
      ...
      break;
    case "bot": {
      const b = spawnBot(ahead);
      b.enemy = "player";
      break;
    }
    case "peter": {
      const b = spawnBot(ahead, 4);
      b.name = "Peterbot1";
      b.hp = 400;
      b.ammo = { light: 999, medium: 999, heavy: 99, shells: 99 };
      b.shield = 100;
      b.weapon = "shotgun";
      b.weapons = ["shotgun", "ar", "sniper"];
      b.heals = 5;
      b.mats = 999;
      b.enemy = "player";
      b.skill = 1;
    ...
    case "alive":
      ...
      break;
    case "nobots":
      for (const b of bots) b.dead = true;
      bots.length = 0;
      break;
    case "cosm":
    ...
    case "xp":
      info("+80,000 XP");
      document.querySelector("#xp .bar").style.background = "linear-gradient(90deg,#c46bff,#c46bff)";
      break;
    case "win":
      endScreen(true);
      toggleDbg(false);
    ...
    case "clear":
      ...
      break;
    case "siege": {
      const c = fortAt(ahead, "stone", 3);
      for (let i = 0; i < 4; i++) {
        const b = spawnBot([c[0] + rand(-3, 3), c[1] + 4.5, c[2] + rand(-3, 3)]);
        b.name = "Defender" + (i + 1);
        b.weapon = i % 2 ? "ar" : "shotgun";
        b.weapons = [b.weapon];
      }
      for (let i = 0; i < 4; i++) {
        const a2 = i / 4 * 6.28;
        const b = spawnBot([c[0] + Math.cos(a2) * 30, c[1] + 1, c[2] + Math.sin(a2) * 30]);
        b.name = "Raider" + (i + 1);
        b.weapon = "ar";
        b.weapons = ["ar"];
        b.target = c;
    ...
    case "meteor":
      event = "meteor";
      eventT = 40;
      banner("METEOR SHOWER", "TAKE COVER", 4);
      break;
    case "edit": {
      const base = Math.floor((ahead[1] + 1) / 4) * 4, cx = Math.floor(ahead[0] / 4) * 4 + 2, cz = Math.floor(ahead[2] / 4) * 4 + 2;
      for (let i = 0; i < 6; i++) {
        W.place("floor", "wood", [cx, base + 4 + i * 4, cz + i * 4], 0);
        W.place("ramp", "wood", [cx, base + i * 4, cz + i * 4], 0);
        W.place("wall", "wood", [cx - 2, base + i * 4, cz + i * 4], 1);
        W.place("wall", "wood", [cx + 2, base + i * 4, cz + i * 4], 1);
        W.place("wall", "wood", [cx, base + i * 4 + 4, cz + i * 4 + 2], 0);
      }
    ...
    case "stop":
      event = null;
      meteors.length = 0;
      banner("EVENT STOPPED", "", 2);
      break;
    case "supply":
      for (let i = 0; i < 6; i++) drops.push({ pos: [P.pos[0] + rand(-50, 50), 130 + rand(0, 30), P.pos[2] + rand(-50, 50)], landed: false });
      banner("SUPPLY DROP PARTY", "6 DROPS INCOMING", 4);
      break;
    case "skydive":
      P.pos = [P.pos[0], terrainH(P.pos[0], P.pos[2]) + 300, P.pos[2]];
      P.vel = [0, 0, 0];
      P.state = "sky";
      toggleDbg(false);
      return;
...
function explode(pos, by, kind = "grenade") {
  if (kind === "boogie") {
    ...
    if (len(sub(P.pos, pos)) < 6 && !P.dead) {
      P.emote = 0;
      P.emoteT = 5;
      P.stunT = 5;
    }
    for (const b of bots) if (!b.dead && len(sub(b.pos, pos)) < 6) {
      b.emote = 0;
      b.emoteT = 5;
      b.stunT = 5;
    }
  ...
  if (kind === "impulse") {
    beep(300, 0.3, "sine", 0.1, -200);
    fx.push({ kind: "dmg", t: 0.6, pos: add(pos, [0, 1.5, 0]), text: "WHOOSH", head: false });
    const push = (e) => {
      const d = sub(add(e.pos, [0, 1, 0]), pos), L = len(d);
      if (L < 7) {
        const k = 26 * (1 - L / 7) + 8;
        e.vel = add(e.vel, add(scale(norm(d), k), [0, k * 0.6, 0]));
        e.grounded = false;
      }
    };
    push(P);
    for (const b of bots) if (!b.dead) push(b);
    return;
  }
  ...
  for (const b of bots) if (!b.dead) {
    ...
    if (d < 5 && dmg(d) > 0) {
      botDamage(b, dmg(d), by);
      if (by === "Player") P.dmg += dmg(d);
    }
  }
...
function updateNades(dt) {
  for (let i = nades.length - 1; i >= 0; i--) {
    ...
    if (next[1] <= g) {
      next[1] = g;
      n.vel = [n.vel[0] * 0.5, -n.vel[1] * 0.35, n.vel[2] * 0.5];
    } else if (W.solids(next[0], next[2], 3).some((b) => next[0] > b.min[0] && next[0] < b.max[0] && next[1] > b.min[1] && next[1] < b.max[1] && next[2] > b.min[2] && next[2] < b.max[2])) {
      n.vel = scale(n.vel, -0.3);
    } else n.pos = next;
...
function updateEvents(dt) {
  if (P.state === "play" && !P.over) {
    P.nextDrop -= dt;
    if (P.nextDrop <= 0) {
      P.nextDrop = 110;
      const a = rand(0, 6.28), rr = rand(0, storm.r * 0.6);
      drops.push({ pos: [storm.c[0] + Math.cos(a) * rr, 160, storm.c[1] + Math.sin(a) * rr], landed: false });
      banner("SUPPLY DROP", "INCOMING", 4);
    }
  }
  for (const d of drops) if (!d.landed) {
    d.pos[1] -= 6 * dt;
    const g = terrainH(d.pos[0], d.pos[2]);
    if (d.pos[1] <= g) {
      d.pos[1] = g;
      d.landed = true;
      chests.push({ pos: [...d.pos], yaw: 0, open: false, drop: true });
    }
  }
  if (event === "meteor") {
    eventT -= dt;
    if (eventT <= 0) event = null;
    if (Math.random() < dt * 1.5) meteors.push({ pos: [P.pos[0] + rand(-60, 60), 140, P.pos[2] + rand(-60, 60)], vel: [rand(-8, 8), -45, rand(-8, 8)] });
  }
  for (let i = meteors.length - 1; i >= 0; i--) {
    ...
    if (m.pos[1] <= W.groundH(m.pos[0], m.pos[2], m.pos[1]) + 0.5) {
      meteors.splice(i, 1);
      beep(60, 0.5, "sawtooth", 0.2, -30);
      fx.push({ kind: "dmg", t: 1, pos: add(m.pos, [0, 2, 0]), text: "BOOM", head: true });
...
const BOT_W = { ar: [0.26, 21, 70, 22], scar: [0.26, 24, 75, 22], burst: [0.3, 21, 70, 22], smg: [0.11, 11, 40, 14], pistol: [0.22, 16, 45, 16], shotgun: [0.9, 58, 14, 6], tac: [0.5, 42, 14, 7], sniper: [1.8, 85, 220, 45], hunting: [1.4, 72, 180, 40], rpg: [3, 95, 120, 30], revolver: [1, 40, 50, 14], silenced: [0.24, 18, 45, 16] };
function los(a, b) {
  const d = sub(b, a), L = len(d);
  const h = W.raycast(a, norm(d), L);
  return !h || h.t >= L - 0.5;
}
const cellOf = (x, z) => [Math.floor(x / 4) * 4 + 2, 0, Math.floor(z / 4) * 4 + 2];
const dirVec = (d) => [Math.sin(d * Math.PI / 2), 0, Math.cos(d * Math.PI / 2)];
const yawToDir = (yaw) => (Math.round(yaw / (Math.PI / 2)) % 4 + 4) % 4;
function botMat(b) {
  return b.mats > 260 ? "metal" : b.mats > 120 ? "stone" : "wood";
}
function botPlace(b, type, pos, dir) {
  if (b.mats < 10 && !D.infMats) return null;
  const p = W.place(type, botMat(b), pos, dir);
  if (p) {
    b.mats -= 10;
    b.buildCd = lerp(0.42, 0.09, b.skill);
    if (Math.random() < 0.1) botHear(pos, 40, b);
  }
  return p;
}
function bestWeaponFor(b, dist) {
  const usable = b.weapons.filter((w) => b.ammo[WEAPONS[w].ammo] > 0);
  if (!usable.length) return null;
  let best = usable[0], bs = 1e9;
  for (const w of usable) {
    const pref = BOT_W[w]?.[3] ?? 20, score = Math.abs(dist - pref) / pref;
    if (score < bs) {
      bs = score;
      best = w;
    }
...
function botHear(pos, radius, who) {
  for (const b of bots) if (!b.dead && b.state === "ground" && b !== who && !b.enemy && len(sub(b.pos, pos)) < radius) {
    if (Math.random() < 0.5 + b.aggression * 0.5) {
      b.memory = [pos[0] + rand(-6, 6), pos[1], pos[2] + rand(-6, 6)];
      b.memoryT = t;
      if (b.aggression > 0.45 && b.weapon) b.mode = "hunt";
    }
  }
}
function updateBot(b, dt) {
  if (b.dead) return;
  b.anim += dt * Math.hypot(b.vel[0], b.vel[2]) * 1.6;
  b.fireCd -= dt;
  b.buildCd -= dt;
  b.retarget -= dt;
  b.voiceCd -= dt;
  ...
  if (b.stunT > 0) {
    b.stunT -= dt;
    b.emoteT = Math.max(b.emoteT, 0.1);
    b.vel[0] *= 0.8;
    b.vel[2] *= 0.8;
    b.vel[1] -= 26 * dt;
    b.grounded = false;
    moveEntity(b, 1.75, dt);
    return;
  }
  if (b.emoteT > 0) {
    b.emoteT -= dt;
    b.vel[0] *= 0.8;
    b.vel[2] *= 0.8;
    if (b.enemy || t - b.lastHit < 2) b.emoteT = 0;
  }
  if (b.state === "island") {
    if (P.islandT < b.joinT) {
      b.pos[1] = -50;
      return;
    }
    b.vel[1] -= 26 * dt;
    b.wanderT -= dt;
    if (!b.target || b.wanderT <= 0 || len(sub(b.target, b.pos)) < 2) {
      b.wanderT = rand(3, 8);
      const a = rand(0, 6.28), rr = rand(5, 35);
      b.target = [ISLAND[0] + Math.cos(a) * rr, 0, ISLAND[2] + Math.sin(a) * rr];
      if (Math.random() < 0.25) {
        b.emoteT = rand(3, 6);
        b.emote = Math.floor(rand(0, 4));
      }
    }
    if (b.emoteT > 0) {
      b.emoteT -= dt;
      ...
      const dx = b.target[0] - b.pos[0], dz = b.target[2] - b.pos[2], L = Math.hypot(dx, dz);
      if (L > 0.5) {
        b.yaw = Math.atan2(dx, dz);
        b.vel[0] = lerp(b.vel[0], dx / L * 5, 0.1);
        b.vel[2] = lerp(b.vel[2], dz / L * 5, 0.1);
      }
      if (b.grounded && Math.random() < dt * 0.3) b.vel[1] = 9;
    }
    if (!b.weapon) {
      b.weapon = "ar";
      b.weapons = ["ar"];
    }
    if (Math.random() < dt * 0.4 && len(sub(b.pos, P.pos)) < 60) {
      const o = bots[Math.floor(rand(0, bots.length))];
      if (o !== b && o.state === "island" && P.islandT >= o.joinT) {
        fx.push({ kind: "tracer", t: 0.06, pos: add(b.pos, [0, 1.5, 0]), to: add(o.pos, [0, 1.2, 0]) });
        beep(200, 0.08, "sawtooth", 0.02, -60);
      }
    }
    b.grounded = false;
    moveEntity(b, 1.75, dt);
  ...
  if (b.state === "bus") {
    b.pos = [...bus.pos];
    if (bus.t > b.dropT || bus.t >= bus.dur) {
      b.state = "sky";
      b.vel = [Math.sin(bus.yaw) * 8, -10, Math.cos(bus.yaw) * 8];
    }
    return;
  }
  const gAbove = b.pos[1] - W.groundH(b.pos[0], b.pos[2], b.pos[1]);
  const fw = [Math.sin(b.yaw), 0, Math.cos(b.yaw)], side = [-Math.cos(b.yaw), 0, Math.sin(b.yaw)];
  const toward = (tgt, spd, face = true) => {
    ...
    if (b.state === "ground") {
      ...
      if (b.probeT <= 0) {
        b.probeT = 0.15;
        b.probeDir = null;
        const eye = add(b.pos, [0, 1, 0]), blocked = (dv) => {
          const h = W.raycast(eye, dv, 2.2);
          return h && h.kind !== "terrain";
        };
        if (blocked(dir)) {
          const l = norm([dir[0] * 0.7 - dir[2] * 0.7, 0, dir[2] * 0.7 + dir[0] * 0.7]), r = norm([dir[0] * 0.7 + dir[2] * 0.7, 0, dir[2] * 0.7 - dir[0] * 0.7]);
          if (!blocked(l)) b.probeDir = l;
          else if (!blocked(r)) b.probeDir = r;
          else if (b.grounded) b.vel[1] = 9;
        }
      }
      if (b.probeDir) dir = b.probeDir;
    }
    if (face) b.yaw = Math.atan2(dir[0], dir[2]);
    b.vel[0] = lerp(b.vel[0], dir[0] * spd, 0.12);
    b.vel[2] = lerp(b.vel[2], dir[2] * spd, 0.12);
    return L;
  };
  if (b.state === "sky") {
    b.vel[1] = Math.max(b.vel[1] - 30 * dt, -40);
    toward(b.land, 18);
    if (gAbove < 40 + b.skill * 30) b.state = "glide";
  } else if (b.state === "glide") {
    b.vel[1] = lerp(b.vel[1], -5.5, 0.05);
    toward(b.land, 11);
  } else {
    b.vel[1] -= 26 * dt;
    if (b.retarget <= 0) {
      b.retarget = lerp(0.5, 0.15, b.skill);
      let found = null, best = 95;
      const eye = add(b.pos, [0, 1.6, 0]);
      const visible = (p, d) => {
        const v = norm(sub(p, eye));
        const facing = v[0] * fw[0] + v[2] * fw[2];
        if (facing < 0.25 && d > 9) return false;
        if (Math.random() > clamp(1.4 - d / 95, 0.15, 1)) return false;
        return los(eye, add(p, [0, 1.2, 0]));
      };
      if (!P.dead && P.state === "play") {
        const d = len(sub(P.pos, b.pos));
        if (d < best && !(P.bush && d > 10 && t - lastShot > 3) && visible(P.pos, d)) {
          best = d;
          found = "player";
        }
      }
      for (const o of bots) if (o !== b && !o.dead && o.state === "ground") {
        const d = len(sub(o.pos, b.pos));
        if (d < best && visible(o.pos, d)) {
          best = d;
          found = o;
        }
      }
      if (found) {
        if (b.enemy !== found) {
          b.seenAt = t;
          if (Math.random() < 0.16) botVoice(b);
        }
        b.enemy = found;
        b.lastSeen = t;
        const q = found === "player" ? P.pos : found.pos;
        b.memory = [...q];
        b.memoryT = t;
        if (b.mode !== "crank" && b.mode !== "box" && b.mode !== "heal" && b.mode !== "rush") b.mode = "fight";
      } else if (b.enemy && t - b.lastSeen > lerp(2.5, 5, b.skill)) {
        b.enemy = null;
        b.mode = b.memory && b.aggression > 0.35 ? "hunt" : "loot";
        b.crank = null;
      }
      if (b.enemy && (b.enemy === "player" ? P.dead : b.enemy.dead)) {
        b.enemy = null;
        b.mode = "loot";
        b.crank = null;
      }
    ...
    if (b.mode !== "heal" && hpTotal < 45 && b.heals > 0 && (!ep || len(sub(ep, b.pos)) > 14 || b.skill > 0.6)) {
      b.mode = "heal";
      b.healT = 0;
      b.boxAt = null;
    } else if (b.mode !== "heal" && !ep && t - b.lastHit > 6 && b.heals > 1 && (b.shield < 50 || b.hp < 70) && Math.random() < dt * 0.3) {
      b.mode = "heal";
      b.healT = 0;
      b.boxAt = null;
    }
    if (ep && b.weapon && b.mode !== "heal") {
      ...
      if (b.mode === "fight" && hpTotal < 50 && b.aggression < 0.5 && L > 25 && !underFire) {
        b.mode = "loot";
        b.enemy = null;
        const away = norm(sub(b.pos, ep));
        b.target = add(b.pos, scale(away, 40));
      }
      const wantsCrank = b.cracked && b.skill > 0.55 && L < 46 && (b.aggression > 0.6 || higher) && (b.mats >= 60 || D.infMats);
      if (b.mode === "fight" && wantsCrank && Math.random() < dt * (0.6 + b.aggression)) {
        b.mode = "crank";
        b.crank = { c: cellOf(b.pos[0], b.pos[2]), L: Math.floor((b.pos[1] + 1) / 4) * 4, d: yawToDir(Math.atan2(ep[0] - b.pos[0], ep[2] - b.pos[2])), t: 0, steps: 0 };
    ...
    if (!ep && (b.mode === "fight" || b.mode === "crank" || b.mode === "rush")) {
      b.mode = b.memory ? "hunt" : "loot";
      b.crank = null;
    }
    ...
    if (fleeing) {
      const away = norm(sub(b.pos, ep));
      toward(add(b.pos, scale(away, 20)), 7.5);
      b.mode = "loot";
    } else if (ep && hpTotal < 30 && b.heals <= 0 && b.mode !== "box" && b.aggression < 0.85) {
      if (b.buildCd <= 0 && b.mats >= 10) {
        const d = yawToDir(Math.atan2(ep[0] - b.pos[0], ep[2] - b.pos[2])), f = dirVec(d), c = cellOf(b.pos[0], b.pos[2]);
        botPlace(b, "wall", [c[0] + f[0] * 2, Math.floor((b.pos[1] + 1) / 4) * 4, c[2] + f[2] * 2], d);
      }
      const away = norm(sub(b.pos, ep));
      toward(add(b.pos, scale(away, 25)), 7.5);
      b.mode = "loot";
      b.crank = null;
    }
    ...
    const aimAndShoot = (L) => {
      ...
      b.yaw += clamp(yawErr, -turnRate * dt, turnRate * dt);
      b.pitch = lerp(b.pitch, desiredPitch, 1 - Math.exp(-lerp(4, 12, b.skill) * dt));
      const want = bestWeaponFor(b, L);
      if (want && want !== b.weapon && b.fireCd <= 0.1) {
        b.weapon = want;
        b.fireCd = 0.5;
        b.shots = 0;
      }
      if (!b.weapon || b.ammo[WEAPONS[b.weapon].ammo] <= 0) {
        if (!want) {
          b.mode = "loot";
          b.lootT = 0;
        }
      ...
      if (!los(add(b.pos, [0, 1.5, 0]), add(ep, [0, 1.2, 0])) && !los(add(b.pos, [0, 1.5, 0]), add(ep, [0, 0.5, 0]))) return;
      b.fireCd = cd * rand(0.9, 1.5) * (b.enemy === "player" ? 1 : 1.4);
      b.ammo[WEAPONS[b.weapon].ammo]--;
      if (++b.shots >= (WEAPONS[b.weapon]?.mag ?? 30)) {
        b.shots = 0;
        b.fireCd = (WEAPONS[b.weapon]?.reload ?? 2.5) * lerp(1.2, 0.8, b.skill);
        if (b.skill > 0.5 && b.mats >= 10 && b.mode === "fight") b.mode = "box";
      }
      ...
      if (Math.random() < 0.2) b.aimDrift = [rand(-1.5, 1.5), rand(-0.75, 0.75), rand(-1.5, 1.5)];
      const from = add(b.pos, [0, 1.5, 0]), to = add(add(ep, [0, 1.2 + rand(-0.45, 0.45), 0]), scale(b.aimDrift, clamp(L / 45, 0.15, 1)));
      if (L < 10 && b.cracked && !los(from, to)) {
        const h = W.raycast(from, norm(sub(to, from)), L);
        if (h && h.kind === "piece") {
          W.damagePiece(h.ref, dmg * 2);
          fx.push({ kind: "tracer", t: 0.06, pos: from, to: h.p });
          fx.push({ kind: "puff", t: 0.22, pos: h.p, col: [0.9, 0.85, 0.7] });
          botHear(b.pos, 60, b);
          return;
        }
      }
      if (ep[1] > b.pos[1] + 3 && L < 22 && b.cracked && Math.random() < 0.35) {
        let low = null;
        for (const p of W.pieces.values()) if ((p.type === "ramp" || p.type === "floor") && len(sub(p.pos, ep)) < 4.5 && (!low || p.pos[1] < low.pos[1])) low = p;
        if (low) {
          W.damagePiece(low, dmg * 1.5);
          fx.push({ kind: "tracer", t: 0.06, pos: from, to: add(low.pos, [0, 0.5, 0]) });
          botHear(b.pos, 60, b);
          return;
        }
      }
      if (b.weapon === "rpg") {
        nades.push({ pos: add(from, scale(norm(sub(to, from)), 1.2)), vel: scale(norm(add(to, [rand(-2, 2) * (1 - b.accuracy), 0, rand(-2, 2) * (1 - b.accuracy)]).map((v, i) => v - from[i])), 34), t: 6, by: b.name, rocket: true });
        botHear(b.pos, 80, b);
        return;
      }
      const hit = Math.random() < acc && los(from, to);
      const tto = hit ? to : add(to, [rand(-3, 3), rand(-2, 2), rand(-3, 3)]);
      fx.push({ kind: "tracer", t: 0.06, pos: from, to: tto });
      if (len(sub(tto, P.pos)) < 30) {
        const hh = W.raycast(from, norm(sub(tto, from)), len(sub(tto, from)) + 4);
        if (hh) fx.push({ kind: "puff", t: 0.22, pos: hh.p, col: [0.75, 0.65, 0.5] });
      }
      if (hit) {
        const head = Math.random() < b.skill * 0.18, n = Math.round(dmg * rand(0.8, 1.1) * (head ? 1.5 : 1));
        if (b.enemy === "player") {
          damage(n, b.name);
          if (head) info("Headshot!");
        } else botDamage(b.enemy, n, b.name);
      } else if (!hit && !los(from, to)) {
        const h = W.raycast(from, norm(sub(to, from)), L);
        if (h && h.kind === "piece") W.damagePiece(h.ref, dmg);
      }
      botHear(b.pos, 60, b);
      if (len(sub(b.pos, P.pos)) < 90) beep(200, 0.08, "sawtooth", 0.03, -60);
    };
    if (fleeing || ep && hpTotal < 30 && b.heals <= 0 && b.mode !== "box" && b.aggression < 0.85) {
      ...
      b.vel[0] = lerp(b.vel[0], want[0], 0.1);
      b.vel[2] = lerp(b.vel[2], want[2], 0.1);
      if (b.grounded && Math.random() < dt * b.skill * 0.6) b.vel[1] = 9;
      if (b.nades > 0 && L > 8 && L < 30 && Math.random() < dt * 0.12 * (1 + b.aggression)) {
        b.nades--;
        const to = sub(ep, b.pos), tl = len(to);
        nades.push({ pos: add(b.pos, [0, 1.6, 0]), vel: add(scale(norm(to), Math.min(20, tl * 0.9)), [0, 6 + tl * 0.15, 0]), t: 2.5, by: b.name });
      }
      if (underFire && b.buildCd <= 0 && b.skill > 0.3 && b.mats >= 10 && Math.random() < dt * (b.cracked ? 4 : 0.8)) {
        const d = yawToDir(Math.atan2(ep[0] - b.pos[0], ep[2] - b.pos[2])), f = dirVec(d), c = cellOf(b.pos[0], b.pos[2]), L0 = Math.floor((b.pos[1] + 1) / 4) * 4;
        botPlace(b, "wall", [c[0] + f[0] * 2, L0, c[2] + f[2] * 2], d);
        if (b.cracked) botPlace(b, "ramp", [c[0], L0, c[2]], d);
      }
    } else if (b.mode === "crank" && ep && b.crank) {
      const k = b.crank, f = dirVec(k.d), r = dirVec((k.d + 1) % 4), rc = [k.c[0] + f[0] * 4, k.L, k.c[2] + f[2] * 4];
      if (k.t === 0 && b.buildCd <= 0) {
        botPlace(b, "floor", [k.c[0], k.L, k.c[2]], 0);
        botPlace(b, "ramp", rc, k.d);
        botPlace(b, "wall", [rc[0] + f[0] * 2, k.L, rc[2] + f[2] * 2], k.d);
        botPlace(b, "wall", [rc[0] + r[0] * 2, k.L, rc[2] + r[2] * 2], (k.d + 1) % 4);
        botPlace(b, "wall", [rc[0] - r[0] * 2, k.L, rc[2] - r[2] * 2], (k.d + 1) % 4);
        if (b.skill > 0.75) botPlace(b, "wall", [k.c[0] - f[0] * 2, k.L, k.c[2] - f[2] * 2], k.d);
        k.t = 0.01;
        b.vel[1] = Math.max(b.vel[1], 8.5);
      }
      k.t += dt;
      const top = [rc[0] + f[0] * 1.6, k.L + 4, rc[2] + f[2] * 1.6];
      const L2 = toward(top, lerp(6, 9.5, b.skill), false);
      const L = len(sub(ep, b.pos));
      aimAndShoot(L);
      if (b.pos[1] > k.L + 3.4 && L2 < 1.2) {
        k.c = rc;
        k.L += 4;
        k.d = (k.d + 1) % 4;
        k.t = 0;
        ...
        k.t = 0;
        k.c = cellOf(b.pos[0], b.pos[2]);
        k.L = Math.floor((b.pos[1] + 1) / 4) * 4;
      }
      if (b.pos[1] > ep[1] + 7 || k.steps > 6 || b.mats < 20 && !D.infMats) {
        botPlace(b, "floor", [k.c[0], k.L, k.c[2]], 0);
        b.mode = "fight";
        b.crank = null;
      }
      ...
      if (b.buildCd <= 0) {
        botPlace(b, "ramp", [c[0], L0, c[2]], d);
        botPlace(b, "floor", [c[0], L0, c[2]], 0);
      }
      ...
      if (!b.boxAt || len(sub(b.boxAt, c)) > 1) {
        b.boxAt = c;
        b.peekWall = null;
      }
      if (b.buildCd <= 0) {
        ...
        botPlace(b, "floor", [c[0], L0 + 4, c[2]], 0);
        botPlace(b, "floor", [c[0], L0, c[2]], 0);
      }
      toward([c[0], L0, c[2]], 4, false);
      b.vel[0] *= 0.7;
      b.vel[2] *= 0.7;
      if (b.mode === "heal") {
        b.healT += dt;
        ...
        if (b.healT > 4) {
          b.healT = 0;
          b.heals--;
          if (b.shield < 100 && Math.random() < 0.5) b.shield = Math.min(100, b.shield + 50);
          else b.hp = Math.min(100, b.hp + 50);
          if (hpTotal + 50 >= 90 || b.heals <= 0) b.mode = ep ? "fight" : "loot";
        }
      } else if (ep) {
        ...
        if (wall && b.cracked && b.peekT <= 0) {
          const open = wall.edit === 0;
          wall.edit = open ? 1 << 4 : 0;
          b.peekWall = wall;
          b.peekT = open ? lerp(1.2, 0.7, b.skill) : lerp(1.4, 0.5, b.skill);
          if (open && b.skill > 0.7 && Math.random() < 0.3) wall.edit = 1 << 1;
        }
        if (wall && wall.edit) aimAndShoot(L);
        if (!underFire && t - b.lastHit > 4 && Math.random() < dt * (0.3 + b.aggression * 0.6)) {
          if (b.peekWall) b.peekWall.edit = 0;
          b.mode = b.aggression > 0.6 ? "crank" : "fight";
          if (b.mode === "crank") b.crank = { c, L: L0, d, t: 0, steps: 0 };
        }
      } else if (t - b.lastHit > 3) {
        if (b.peekWall) b.peekWall.edit = 0;
        b.mode = "loot";
      }
    } else {
      ...
      let chest = null, cdist = b.weapon ? 30 : 120;
      for (const c of chests) if (!c.open && !b.ignore.has(c)) {
        const d = len(sub(c.pos, b.pos));
        if (d < cdist) {
          cdist = d;
          chest = c;
        }
      }
      const tgt = chest ?? null;
      if (tgt && tgt === b.tryRef) {
        if (len(sub(tgt.pos, b.pos)) < 14) b.tryT += dt;
        if (b.tryT > 8) {
          b.ignore.add(tgt);
          b.tryRef = null;
          b.tryT = 0;
      ...
      for (const g of items) {
        if (b.ignore.has(g)) continue;
        const k = g.item.kind;
        const need = isWeapon(k) ? !b.weapon || b.weapons.length < 3 && !b.weapons.includes(k) || b.weapon === "smg" && k !== "smg" : k === "ammo" ? b.weapon !== null && Object.values(b.ammo).reduce((a, v) => a + v, 0) < 60 : k === "boogie" || k === "impulse" || k === "launchpad" || k === "bushItem" ? false : k === "grenade" ? b.nades < 3 : b.heals < 3;
        if (!need) continue;
        const d = len(sub(g.pos, b.pos));
        if (d < idist) {
          idist = d;
          item = g;
        }
      ...
      if (out) {
        b.mode = "rotate";
        if (!b.target || Math.hypot(b.target[0] - sc[0], b.target[2] - sc[1]) > srad * 0.5) {
          const a = rand(0, 6.28), rr = rand(0, srad * 0.5);
          b.target = [sc[0] + Math.cos(a) * rr, 0, sc[1] + Math.sin(a) * rr];
        }
        toward(b.target, 6.5);
      } else if (b.mode === "hunt" && b.memory && b.weapon) {
        const L = toward(b.memory, 6.5);
        if (L < 3) {
          b.memory = null;
          b.mode = "loot";
        } else if (L < 40 && b.cracked && b.buildCd <= 0 && b.mats >= 30 && Math.random() < dt * 0.5) {
          const d = yawToDir(b.yaw), f = dirVec(d), c = cellOf(b.pos[0] + f[0] * 2.5, b.pos[2] + f[2] * 2.5), L0 = Math.floor((b.pos[1] + 1) / 4) * 4;
          botPlace(b, "ramp", [c[0], L0, c[2]], d);
          botPlace(b, "wall", [c[0] + f[0] * 2, L0 + 4, c[2] + f[2] * 2], d);
          b.vel[1] = Math.max(b.vel[1], 7);
        }
        ...
        if (L < 2.6) {
          b.vel[0] *= 0.6;
          b.vel[2] *= 0.6;
          if (b.interactRef !== chest) {
            b.interactRef = chest;
            b.interactT = 1.2;
          } else b.interactT -= dt;
          if (b.interactT <= 0) {
            chest.open = true;
            const pool = ["ar", "burst", "smg", "shotgun", "sniper", "tac", "hunting", "scar", "pistol"];
            const k = pool[Math.floor(rand(0, pool.length))];
            if (!b.weapons.includes(k) && b.weapons.length < 3) b.weapons.push(k);
            b.weapon = b.weapon ?? k;
            b.heals = Math.min(4, b.heals + 1);
            b.shield = Math.min(100, b.shield + 25);
            b.mats = Math.min(999, b.mats + 30);
            b.ammo.medium += 30;
            b.ammo.light += 30;
            b.ammo.shells += 5;
            b.ammo.heavy += 3;
      ...
      } else if (!b.weapon && b.lootT <= 0) {
        let spot = null, sd = 120;
        for (const l of W.lootSpots) {
          const d = len(sub(l, b.pos));
          if (d < sd) {
            sd = d;
            spot = l;
          }
        ...
        if (spot) {
          ...
          if (L < 7) {
            ...
            if (b.interactT > 4) {
              b.interactT = 0;
              b.lootT = 20;
              const pool = ["pistol", "smg", "ar", "burst", "shotgun", "tac", "revolver"];
              const k = pool[Math.floor(rand(0, pool.length))];
              b.weapons.push(k);
              b.weapon = k;
              const a = WEAPONS[k].ammo;
              b.ammo[a] += a === "heavy" ? 5 : a === "shells" ? 12 : 40;
              b.heals++;
            }
          }
        }
      } else if (b.mats < 200 && !b.farmRef && b.weapon) {
        let best = null, bd = 90;
        for (const c of W.props) if (!c.dead && c.type !== "bush") {
          const d = len(sub(c.pos, b.pos));
          if (d < bd) {
            bd = d;
            best = c;
        ...
        if (best) b.farmRef = best;
        else b.mode = "rotate";
      } else if (b.farmRef) {
        const q = b.farmRef;
        if (q.dead || b.mats >= 400) {
          b.farmRef = null;
        } else {
          const L = toward(q.pos, 5.5);
          if (L < q.r + 2.2) {
            b.vel[0] *= 0.6;
            b.vel[2] *= 0.6;
            b.farmT -= dt;
            if (b.farmT <= 0) {
              b.farmT = 0.5;
              b.mats = Math.min(999, b.mats + 10);
              q.hp -= 50;
              if (len(sub(q.pos, P.pos)) < 40) beep(300, 0.08, "triangle", 0.03);
              if (q.hp <= 0) {
                q.dead = 30;
                dirtyProp(q);
                b.farmRef = null;
        ...
        if (item === b.tryRef) {
          if (len(sub(item.pos, b.pos)) < 14) b.tryT += dt;
          if (b.tryT > 8) {
            b.ignore.add(item);
            b.tryRef = null;
            b.tryT = 0;
        ...
        const L = toward(item.pos, 5.8);
        if (L < 1.6) {
          const k = item.item.kind;
          if (isWeapon(k)) {
            if (!b.weapons.includes(k)) {
              if (b.weapons.length >= 3) b.weapons.shift();
              b.weapons.push(k);
            }
            b.weapon = k;
            const a = WEAPONS[k].ammo;
            b.ammo[a] += a === "heavy" ? 5 : a === "shells" ? 10 : 30;
          } else if (k === "ammo") {
            b.ammo.light += 18;
            b.ammo.medium += 12;
            b.ammo.shells += 4;
            b.ammo.heavy += 2;
          } else if (k !== "grenade") b.heals++;
          if (k === "grenade") b.nades += 3;
        ...
        if (!b.target || b.wanderT <= 0 || len(sub(b.target, b.pos)) < 3) {
          ...
          if (late || Math.random() < 0.3) {
            const a = rand(0, 6.28), rr = rand(0, storm.r * 0.55);
            b.target = [storm.c[0] + Math.cos(a) * rr, 0, storm.c[1] + Math.sin(a) * rr];
          } else {
            const spot = W.lootSpots[Math.floor(rand(0, W.lootSpots.length))];
            b.target = len(sub(spot, b.pos)) < 90 ? [...spot] : [b.pos[0] + rand(-40, 40), 0, b.pos[2] + rand(-40, 40)];
          }
        }
        if (b.emoteT <= 0 && Math.random() < dt * 0.012) {
          b.emoteT = rand(3, 5);
          b.emote = Math.floor(rand(0, 4));
          if (len(sub(b.pos, P.pos)) < 40) emoteJingle(b.emote);
        }
        if (b.emoteT <= 0) toward(b.target, 5.2);
      }
      if (b.heals <= 0 && Math.random() < dt * 0.02) b.heals = 1;
    }
    for (const n of nades) if (!n.rocket && n.t < 2 && len(sub(n.pos, b.pos)) < 6 && b.skill > 0.3) {
      const away = norm(sub(b.pos, n.pos));
      b.vel[0] = away[0] * 8;
      b.vel[2] = away[2] * 8;
      if (b.grounded) b.vel[1] = 8;
      break;
    }
    if (Math.hypot(b.vel[0], b.vel[2]) > 1.5 && len(sub(b.pos, b.lastPos)) < 0.05 * 1) b.stuckT += dt;
    else b.stuckT = 0;
    if (b.stuckT > 0.6 && b.grounded) {
      b.vel[1] = 9;
      if (b.stuckT > 2) {
        b.target = null;
        b.stuckT = 0;
        if (b.cracked && b.buildCd <= 0 && b.mats >= 10) {
          const d = yawToDir(b.yaw), f = dirVec(d), c = cellOf(b.pos[0] + f[0] * 2.5, b.pos[2] + f[2] * 2.5);
  ...
  b.grounded = false;
  if (moveEntity(b, 1.75, dt) && b.state !== "ground") {
    b.state = "ground";
    b.mode = "loot";
  }
}
...
function frame(now) {
  ...
  fpsT += dt;
  if (fpsT > 0.5) {
    fpsV = Math.round(fpsN / fpsT);
    fpsN = 0;
    fpsT = 0;
    if (P.state === "play") {
      lowT = fpsV < 30 ? lowT + 0.5 : 0;
      if (lowT >= 3) {
        lowT = 0;
        const step = S.shadows > 1 ? S.shadows = 1 : S.grass > 0 ? S.grass = 0 : S.scale > 0.75 ? S.scale = 0.75 : S.shadows > 0 ? S.shadows = 0 : S.scale > 0.6 ? S.scale = 0.6 : S.viewDist > 0 ? S.viewDist = 0 : -1;
        if (step !== -1) info("Low FPS: quality lowered (Settings > Video)");
      }
    }
  }
  const key = (c) => pressed.has(c);
  const sun = norm([0.55, 0.62, 0.35]), aspect = innerWidth / innerHeight;
  ...
  const curGpButtons = /* @__PURE__ */ new Set();
  let gpWish = [0, 0, 0];
  if (gp) {
    ...
    const isB = (i) => {
      ...
      return b ? typeof b === "object" ? b.pressed : b === 1 : false;
    };
    for (let i = 0; i < gp.buttons.length; i++) if (isB(i)) curGpButtons.add(i);
    const justB = (i) => curGpButtons.has(i) && !gpPrev.has(i);
    const lt = (gp.buttons[6]?.value ?? 0) > 0.25 || gp.axes[4] !== void 0 && gp.axes[4] > 0.2;
    const rt = (gp.buttons[7]?.value ?? 0) > 0.25 || gp.axes[5] !== void 0 && gp.axes[5] > 0.2;
    ...
    if (Math.abs(rx) > 0 || Math.abs(ry) > 0) {
      const padSens = 650 * dt * S.padSens * (P.scoped ? 0.4 : P.ads ? 0.6 : 1);
      mouse.dx += rx * padSens;
      mouse.dy += ry * padSens;
    }
    if (rt) mouse.l = true;
    ...
    if (isB(10)) keys.add("ShiftLeft");
    if (isB(11)) keys.add("ControlLeft");
    if (isB(0)) {
      keys.add("Space");
      if (justB(0)) pressed.add("Space");
    }
    if (justB(1)) {
      pressed.add("KeyZ");
      if (P.editing) P.editing = null;
    }
    if (justB(2)) {
      pressed.add("KeyE");
      pressed.add("KeyR");
    ...
    if (justB(4)) {
      if (P.build) {
        const pcs = ["wall", "floor", "ramp", "pyramid"], i = pcs.indexOf(P.piece);
        P.piece = pcs[(i + 3) % 4];
        rumble(40, 0.2, 0.2);
      } else {
        const n = P.inv.length;
        P.slot = P.slot < 0 ? 0 : (P.slot + n - 1) % n;
        P.build = false;
        rumble(40, 0.2, 0.2);
      }
    }
    if (justB(5)) {
      if (P.build) {
        ...
        const n = P.inv.length;
        P.slot = P.slot < 0 ? 0 : (P.slot + 1) % n;
        P.build = false;
        rumble(40, 0.2, 0.2);
      }
    }
    if (justB(12)) pressed.add("KeyM");
    if (justB(13)) pressed.add("KeyB");
    if (justB(14)) {
      if (P.build) pressed.add("MR");
    }
    if (justB(15)) {
      if (P.build) {
        P.rampRot = (P.rampRot + 1) % 4;
        rumble(40, 0.2, 0.2);
      }
    }
    if (justB(8)) pressed.add("KeyM");
    if (justB(9)) {
      if (P.state === "lobby") $("btnPlay").click();
      else toggleDbg();
    }
    if (P.state === "lobby" && (justB(0) || justB(9))) {
      AC ??= new AudioContext();
      startMatch();
      rumble(180, 0.5, 0.5);
    }
    if (P.state === "lobby" && (justB(1) || justB(3))) {
      $("btnSkin").click();
      rumble(80, 0.3, 0.3);
    }
  }
  if (P.state === "lobby" && GALLERY) {
    ...
    const dist = (5 + n * 2.2) / Math.min(1, aspect), cam = [Math.sin(ang) * dist, 3 + n * 0.4, Math.cos(ang) * dist];
    VP = mul(perspective(0.7, aspect, 0.1, 300), lookAt(cam, [0, 1.6, 0]));
    R.draw(M.pad, trs([0, -0.4, 0], 0, 0, [n * 1.6, 1, 2]));
    names.forEach((nm, i) => {
      const x = (i - (n - 1) / 2) * sp;
      if (nm.startsWith("skin")) drawChar(CHARS[+nm.slice(4) % CHARS.length], trs([x, 0, 0], ang), { anim: 0, speed: 0, grounded: true, pitch: 0, pose: "lobby" });
      else if (nm.startsWith("house")) {
        const idx = +nm.slice(5);
        R.draw(W.houseMeshes[idx % W.houseMeshes.length], trs([x, 0, 0], ang, 0, 0.35));
      } else if (M[nm]) R.draw(M[nm], trs([x, 0, 0], ang * 2, 0, nm === "bus" || nm === "balloon" ? 0.4 : 1));
    });
    R.flush({ pos: cam, fwd: norm(sub([0, 1.6, 0], cam)), fov: 0.7, aspect }, VP, norm([0.3, 0.8, 0.6]), [0, 0, 0], t, true, 20 + n * 3);
    pressed.clear();
    return;
  }
  if (P.state === "lobby") {
    const a = t * 0.25, cam = [Math.sin(a) * 0.4, 1.5, 7.2];
    VP = mul(perspective(0.55, aspect, 0.1, 100), lookAt(cam, [0, 1.25, 0]));
    const ch = P.skin === 0 ? LOBBY_CHAR : CHARS[P.skin];
    R.draw(M.pad, trs([0, -0.4, 0]), [1, 1, 1]);
    R.draw(M.pad, trs([-4.2, -0.6, -1.5]));
    R.draw(M.pad, trs([4, -0.6, -1.5]));
    R.draw(M.pad, trs([6.5, -0.7, -2.5]));
    drawChar(ch, trs([0, 0, 0], Math.sin(t * 0.5) * 0.08), { anim: 0, speed: 0, grounded: true, pitch: 0, pose: "lobby" });
    R.flush({ pos: cam, fwd: norm(sub([0, 1.35, 0], cam)), fov: 0.55, aspect }, VP, norm([0.3, 0.8, 0.6]), [0, 0, 0], t, false, 12);
    pressed.clear();
    return;
  }
  const sens = 32e-4 * (P.scoped ? S.scopeSens : P.ads ? S.adsSens : 1);
  P.yaw -= mouse.dx * sens * S.sensX;
  P.pitch = clamp(P.pitch - mouse.dy * sens * S.sensY * (S.invertY ? -1 : 1), -1.5, 1.5);
  mouse.dx = mouse.dy = 0;
  if (key("KeyL")) {
    toLobby();
    pressed.clear();
  ...
  for (const pd of pads) {
    pd.t += dt;
    if (P.state === "play" && !P.dead && Math.hypot(P.pos[0] - pd.pos[0], P.pos[2] - pd.pos[2]) < 1.6 && Math.abs(P.pos[1] - pd.pos[1]) < 1.2) {
      P.state = "sky";
      P.vel = [P.vel[0], 30, P.vel[2]];
      P.pos[1] += 0.5;
      beep(700, 0.3, "sine", 0.1, 600);
      rumble(200, 0.5, 0.8);
    }
    for (const b of bots) if (!b.dead && b.state === "ground" && Math.hypot(b.pos[0] - pd.pos[0], b.pos[2] - pd.pos[2]) < 1.6 && Math.abs(b.pos[1] - pd.pos[1]) < 1.2) {
      b.state = "sky";
      b.vel = [b.vel[0], 30, b.vel[2]];
      b.pos[1] += 0.5;
      b.land = b.target ?? b.land;
    }
  }
  if (key("KeyM")) H.bigmap.style.display = H.bigmap.style.display === "flex" ? "none" : "flex";
  if (key("KeyB") && P.state === "play" && !P.dead) {
    if (EW.style.display === "flex") {
      EW.style.display = "none";
      startEmote(lastEmote);
    } else {
  ...
  if (P.stunT > 0) {
    P.stunT -= dt;
    P.emoteT = Math.max(P.emoteT, 0.1);
    keys.delete("KeyW");
    keys.delete("KeyA");
    keys.delete("KeyS");
    keys.delete("KeyD");
  ...
  if (P.emoteT > 0) {
    P.emoteT -= dt;
    if (P.stunT <= 0 && (Math.hypot(P.vel[0], P.vel[2]) > 1 || mouse.l)) P.emoteT = 0;
  }
  if (key("KeyT")) P.thirdPerson = !P.thirdPerson;
  P.matchT += dt;
  storm.phaseT = Math.max(0, storm.phaseT - dt);
  if (storm.shrinking) {
    const k = 1 - storm.phaseT / storm.shrinkT;
    storm.r = lerp(storm.from.r, storm.to.r, k);
    storm.c = [lerp(storm.from.c[0], storm.to.c[0], k), lerp(storm.from.c[1], storm.to.c[1], k)];
    if (storm.phaseT <= 0) {
      storm.shrinking = false;
  ...
  if (fadeT > 0) {
    fadeT -= dt;
    H.fade.style.opacity = String(clamp(fadeT / fadeDur * 1.6, 0, 1));
    if (fadeT <= 0) H.fade.style.opacity = "0";
  }
  if (P.matchT > 20 && Math.random() < dt * 0.12 && P.alive > bots.filter((b) => !b.dead).length + 1) {
    P.alive--;
    addFeed(`${botName()} eliminated <span class="v">${botName()}</span>`);
  }
  if (P.state === "island") {
    P.islandT += dt;
    P.hp = 100;
    ...
    let joined = 1;
    for (const b of bots) {
      if (P.islandT >= b.joinT && !b.joinedFeed) {
        b.joinedFeed = true;
        addFeed(`${b.name} has joined`);
      }
      if (P.islandT >= b.joinT) joined++;
    }
    ...
    if (left <= 0) {
      ...
      P.shield = 0;
      W.clearPieces();
      for (let i = items.length - 1; i >= 0; i--) if (items[i].pos[2] < -700) items.splice(i, 1);
      for (const b of bots) {
        b.state = "bus";
        b.pos = [...bus.a];
        b.hp = 100;
  ...
  if (bus.t >= 0 && bus.t < bus.dur) {
    ...
    const k = bus.t / bus.dur;
    bus.pos = add(bus.a, scale(sub(bus.b, bus.a), k));
  }
  if (P.state === "bus") {
    P.pos = [bus.pos[0], bus.pos[1] + 3, bus.pos[2]];
    P.vel = [0, 0, 0];
    if (key("KeyB") && !P.thanked) {
      P.thanked = true;
      addFeed(`<span class="me">Player</span> has thanked the bus driver`);
      for (let i = 0; i < 3; i++) setTimeout(() => addFeed(`${botName()} has thanked the bus driver`), 400 + i * 700);
    }
    if (key("Space") && bus.t > 4 || bus.t >= bus.dur) {
      P.state = "sky";
      P.vel = [Math.sin(bus.yaw) * 8, -5, Math.cos(bus.yaw) * 8];
      P.pos = [bus.pos[0], bus.pos[1] - 1, bus.pos[2]];
      beep(300, 0.3, "sine", 0.05, -200);
    }
  } else {
    let wish = [0, 0, 0];
    if (keys.has("KeyW")) wish = add(wish, fwd());
    if (keys.has("KeyS")) wish = sub(wish, fwd());
    if (keys.has("KeyD")) wish = add(wish, right());
    if (keys.has("KeyA")) wish = sub(wish, right());
    if (len(gpWish) > 0) wish = add(wish, gpWish);
    if (len(wish) > 0) wish = norm(wish);
    P.crouch = (P.state === "play" || P.state === "island") && keys.has("ControlLeft");
    P.sprint = keys.has("ShiftLeft") && !P.crouch;
    const gAbove = P.pos[1] - W.groundH(P.pos[0], P.pos[2], P.pos[1]);
    if (P.state === "sky") {
      P.vel[1] = Math.max(P.vel[1] - 30 * dt, keys.has("KeyW") ? -55 : -35);
      P.vel[0] = lerp(P.vel[0], wish[0] * 18, 0.03);
      P.vel[2] = lerp(P.vel[2], wish[2] * 18, 0.03);
      if (gAbove < 55 || key("Space")) {
        P.state = "glide";
        beep(800, 0.2, "sine", 0.06, -300);
      }
    } else if (P.state === "glide") {
      P.vel[1] = lerp(P.vel[1], -5.5, 0.05);
      const f = fwd();
      P.vel[0] = lerp(P.vel[0], f[0] * 11 + wish[0] * 4, 0.05);
      P.vel[2] = lerp(P.vel[2], f[2] * 11 + wish[2] * 4, 0.05);
    } else if (P.swim && !D.fly) {
      const spd = P.sprint ? 5 : 3.8;
      P.vel[0] = lerp(P.vel[0], wish[0] * spd, 0.08);
      P.vel[2] = lerp(P.vel[2], wish[2] * spd, 0.08);
      P.vel[1] = lerp(P.vel[1], (-1.25 - P.pos[1]) * 4, 0.15);
      if (key("Space")) P.vel[1] = 5;
      P.build = false;
      P.editing = null;
    } else {
      const spd = D.fly ? 22 : P.crouch ? 3 : P.sprint ? 8.5 : 5.5, accel = P.grounded || D.fly ? 14 : 4;
      P.vel[0] = lerp(P.vel[0], wish[0] * spd, 1 - Math.exp(-accel * dt));
      P.vel[2] = lerp(P.vel[2], wish[2] * spd, 1 - Math.exp(-accel * dt));
      if (D.fly) P.vel[1] = lerp(P.vel[1], (keys.has("Space") ? 14 : 0) - (keys.has("ControlLeft") ? 14 : 0), 0.2);
      else {
        P.vel[1] -= (D.lowGrav ? 8 : 26) * dt;
        if (key("Space") && P.grounded) {
          P.vel[1] = D.lowGrav ? 7 : 9.5;
          P.grounded = false;
        }
      }
    }
    ...
    P.hurtCd -= dt;
    if (P.state === "play" && Math.hypot(P.pos[0] - storm.c[0], P.pos[2] - storm.c[1]) > storm.r && P.hurtCd <= 0) {
      damage(storm.phase > 3 ? 5 : storm.phase > 1 ? 2 : 1, "The storm");
      P.hurtCd = 1;
    }
  }
  if (P.dyingT > 0) P.dyingT -= dt;
  if (P.dead && P.dyingT <= 0) {
  ...
  const head = add(P.pos, [0, eyeH(), 0]);
  camFwd = look();
  const baseFov = 2 * Math.atan(Math.tan(S.fov * Math.PI / 360) / Math.max(1, aspect));
  ...
  if ((P.state === "play" || P.state === "island") && !P.over && !P.dead && !dbgOpen()) {
    ...
    for (const [k, p] of [["KeyQ", "wall"], ["KeyG", "floor"], ["KeyF", "ramp"], ["AltLeft", "pyramid"]]) if (key(k)) {
      P.piece = p;
      P.build = true;
    }
    if (key("Backquote")) {
    ...
    for (let i = 0; i < 5; i++) if (key("Digit" + (i + 1)) && P.slot !== i) {
      P.slot = i;
      P.build = false;
      P.reload = 0;
      P.fireCd = 0.35;
    ...
    if (key("KeyX")) {
      if (P.editing) {
        ...
        const h = W.raycast(camPos, camFwd, 10);
        if (h && h.kind === "piece" && TILES(h.ref.type)) {
          P.editing = h.ref;
          P.editMask = P.editing.edit;
          P.build = false;
        }
      }
    }
    if (P.editing) {
      if (key("MR")) {
        ...
        if (h) {
          const tile = W.tileAt(pc, add(camPos, scale(camFwd, h.t + 0.05)));
          if (tile >= 0 && (key("ML") || !(P.editMask & 1 << tile))) {
            P.editMask ^= 1 << tile;
            P.fireCd = 0.12;
    ...
    if (P.reload > 0) {
      P.reload -= dt;
      if (P.reload <= 0 && it && isWeapon(it.kind)) {
        const w = WEAPONS[it.kind], n = Math.min(w.mag - it.mag, P.ammo[w.ammo]);
        it.mag += n;
        P.ammo[w.ammo] -= n;
      }
    }
    if (P.editing) {
      ...
      if (mouse.l && P.fireCd <= 0 && (P.mats[P.mat] >= 10 || D.infMats) && !W.pieces.has(World.key(bt.type, bt.pos, bt.dir))) {
        ...
        if (Math.random() < 0.15) botHear(bt.pos, 40, "player");
      }
    } else if (P.slot < 0 || !it) {
      if (mouse.l && P.swing <= 0.05 && P.fireCd <= 0) {
        swingPickaxe();
        P.fireCd = 0.45;
      }
    } else if (isWeapon(it.kind)) {
      const w = WEAPONS[it.kind];
      if ((w.auto ? mouse.l : key("ML")) && P.fireCd <= 0 && P.reload <= 0) {
        if (it.mag > 0) shoot(it);
        else if (P.ammo[w.ammo] > 0) P.reload = w.reload;
      ...
      if (key("ML") && P.grounded) {
        const f = fwd(), pp = [P.pos[0] + f[0] * 2.5, 0, P.pos[2] + f[2] * 2.5];
        pp[1] = W.groundH(pp[0], pp[2], P.pos[1]);
        pads.push({ pos: pp, t: 0 });
        if (--it.count <= 0) P.inv[P.slot] = null;
        P.fireCd = 0.5;
        beep(900, 0.15, "triangle", 0.06, 300);
      }
      ...
      const hw = W.raycast(camPos, camFwd, 25);
      const water = hw && hw.kind === "terrain" && hw.p[1] < -0.2;
      if (key("ML") && water && P.fishing <= 0) {
        P.fishing = 2.5;
        P.useT = 2.5;
        P.useDur = 2.5;
      ...
      if (P.fishing > 0) {
        ...
        if (P.fishing <= 0) {
          const r = Math.random();
          const k = r < 0.55 ? "fish" : r < 0.75 ? "shotgun" : r < 0.9 ? "ar" : "sniper";
          dropItem(mkItem(k, k === "fish" ? 2 : 1, k === "fish" ? 3 : Math.max(2, Math.floor(rand(2, 5)))), add(P.pos, scale(fwd(), 1.5)));
          info("Caught a " + (isWeapon(k) ? WEAPONS[k].name : "Flopper") + "!");
          beep(800, 0.3, "sine", 0.08, 300);
        }
      }
    } else {
    ...
    for (let i = items.length - 1; i >= 0; i--) if (items[i].item.kind === "ammo" && len(sub(items[i].pos, P.pos)) < 1.6) {
      P.ammo.light += 18;
      P.ammo.medium += 12;
      P.ammo.shells += 4;
      P.ammo.heavy += 2;
      items.splice(i, 1);
      beep(700, 0.06, "sine", 0.05, 200);
      info("+ ammo");
    }
    let near = null, nd = 2.4;
    for (const g of items) {
      if (g.item.kind === "ammo") continue;
      const d = len(sub(g.pos, P.pos));
      if (d < nd) {
        nd = d;
        near = g;
      }
    ...
    let nearChest = null;
    for (const c of chests) if (!c.open && len(sub(c.pos, P.pos)) < 2.8) nearChest = c;
    if (near) {
      H.info.textContent = `[E] ${isWeapon(near.item.kind) ? WEAPONS[near.item.kind].name : CONS[near.item.kind].name}`;
      H.info.style.display = "block";
      infoT = Math.max(infoT, 0.05);
    } else if (nearChest) {
      H.info.textContent = "[E] Open chest";
      H.info.style.display = "block";
      infoT = Math.max(infoT, 0.05);
    }
    if (key("KeyE")) {
      if (nearChest) {
        ...
        if (nearChest.drop) {
          dropItem(mkItem("rpg", 1, 4), add(nearChest.pos, [0, 0.3, 0]), 1.8);
          dropItem(mkItem("rod"), add(nearChest.pos, [0, 0.3, 0]), 1.4);
          dropItem(mkItem("sniper", 1, 4), add(nearChest.pos, [0, 0.3, 0]), 1.6);
        }
        dropItem(mkItem(["shieldPot", "bandage", "miniShield", "chug", "grenade", "boogie", "impulse"][Math.floor(rand(0, 7))], 3), add(nearChest.pos, [0, 0.3, 0]), 1.2);
        P.ammo.medium += 30;
        P.ammo.light += 30;
        P.ammo.shells += 5;
        P.ammo.heavy += 3;
        ...
        if (same >= 0) P.inv[same].count += near.item.count;
          ...
          if (s < 0) {
            s = Math.max(0, P.slot);
            dropItem(P.inv[s], near.pos);
          }
          P.inv[s] = near.item;
          if (P.slot < 0 || !P.inv[P.slot]) P.slot = s;
        }
  ...
  if (infoT > 0) {
    infoT -= dt;
    if (infoT <= 0) H.info.style.display = "none";
  }
  ...
  for (const tc of W.terrainChunks) {
    const dx = tc.c[0] - camPos[0], dz = tc.c[2] - camPos[2], dist = Math.hypot(dx, dz);
    if (P.state === "play" && dist > tc.r && (dist - tc.r > [420, 600, 900][S.viewDist] || dx * camFwd[0] + dz * camFwd[2] < -tc.r)) continue;
    R.draw(tc.mesh, trs([0, 0, 0]), [1, 1, 1], 1, 5);
  }
  if (P.state === "play" && S.grass > 0) {
    const cx = Math.floor(P.pos[0] / 24), cz = Math.floor(P.pos[2] / 24), gr = S.grass > 1 ? 2 : 1;
    for (let i = -gr; i <= gr; i++) for (let j = -gr; j <= gr; j++) R.draw(W.grassChunk(R, cx + i, cz + j), trs([0, 0, 0]), [1, 1, 1], 1, 5, false, true);
  }
  R.draw(M.mountains, trs([0, 0, 0]), [1, 1, 1], 1, 0, false);
  if (P.state === "island") {
    R.draw(W.island, trs([0, 0, 0]), [1, 1, 1], 1, 5);
    R.draw(M.mountains, trs([ISLAND[0], 0, ISLAND[2]]), [1, 1, 1], 1, 0, false);
    R.draw(M.bus, trs([ISLAND[0] - 10, 5.6, ISLAND[2] + 36], 1.2));
    R.draw(M.balloon, trs([ISLAND[0] - 10, 21.6, ISLAND[2] + 36], 1.2));
  }
  R.draw(M.water, trs([0, -0.25, 0]), [1, 1, 1], 0.82, 6, false);
  const cull = P.state === "play" ? [130, 190, 320][S.viewDist] : 900;
  const vis = (p) => Math.abs(p[0] - camPos[0]) < cull && Math.abs(p[2] - camPos[2]) < cull && (p[0] - camPos[0]) * camFwd[0] + (p[2] - camPos[2]) * camFwd[2] > -18;
  {
    const r = Math.ceil((cull + PC) / PC), kx = Math.floor(camPos[0] / PC), kz = Math.floor(camPos[2] / PC);
    for (let i = -r; i <= r; i++) for (let j = -r; j <= r; j++) {
      const c = [(kx + i + 0.5) * PC, 0, (kz + j + 0.5) * PC];
      if (Math.abs(c[0] - camPos[0]) > cull + PC || Math.abs(c[2] - camPos[2]) > cull + PC || (c[0] - camPos[0]) * camFwd[0] + (c[2] - camPos[2]) * camFwd[2] < -PC) continue;
      const cell = propCell(kx + i + "," + (kz + j));
      if (cell) R.draw(cell.mesh, trs([0, 0, 0]));
    }
  }
  ...
  for (const s of W.statics) if (!s.dead && !s.baked && vis(s.pos)) {
    const sh = s.shake || 0, sp = sh ? [s.pos[0] + Math.sin(t * 95) * sh * 0.12, s.pos[1], s.pos[2] + Math.cos(t * 81) * sh * 0.12] : s.pos;
  ...
  for (const p of W.pieces.values()) {
    ...
    const age = performance.now() / 1e3 - p.born, k = clamp(age / 0.18, 0, 1), sc = 0.6 + 0.4 * k, mesh = p.edit ? editedMesh(p.type, p.mat, p.edit) : M[`${p.type}_${p.mat}`];
    const tint = k < 1 ? [0.6 + 0.4 * k, 0.8 + 0.2 * k, 1.3 - 0.3 * k] : p.hp < p.maxHp ? [1, 0.7 + 0.3 * p.hp / p.maxHp, 0.7 + 0.3 * p.hp / p.maxHp] : [1, 1, 1];
  ...
  if (P.editing) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (pc.type === "wall") {
        ...
        local = [-2 + (col + 0.5) * T, (row + 0.5) * T, 0];
        size = [T * 0.9, T * 0.9, 0.4];
      } else {
        local = [i % 2 ? 1 : -1, 0.05, i > 1 ? 1 : -1];
        size = [1.8, 0.3, 1.8];
      }
  ...
  for (const c of chests) {
    R.draw(c.open ? M.chestOpen : M.chest, trs(c.pos, c.yaw), c.open ? [1, 1, 1] : [1.15, 1.1, 0.9]);
    if (!c.open && len(sub(c.pos, camPos)) < 60) R.draw(M.glow, trs(c.pos, 0, 0, 1 + Math.sin(t * 3) * 0.08), [1, 0.85, 0.3], 0.16, 7, false);
  }
  const RAR_COL = [[0.7, 0.7, 0.7], [0.3, 0.9, 0.3], [0.3, 0.6, 1], [0.75, 0.35, 1], [1, 0.75, 0.2]];
  for (const g of items) {
    if (g.item.kind === "ammo") R.draw(M.ammo, trs(g.pos, 0.6, 0, 1.6));
    else {
      R.draw(M[g.item.kind], trs(add(g.pos, [0, 0.6 + Math.sin(t * 3) * 0.1, 0]), t * 1.5, 0, 1.3));
      if (len(sub(g.pos, camPos)) < 45) {
        const rc = RAR_COL[g.item.rar] ?? RAR_COL[0];
        R.draw(M.glow, trs(g.pos, 0, 0, [0.7, 0.35, 0.7]), rc, 0.22, 7, false);
        R.draw(M.beam, trs(g.pos), rc, 0.12, 7, false);
      }
    }
  }
  for (const f of fx) if (f.kind === "puff") R.draw(M.glow, trs(f.pos, 0, 0, 0.12 + (0.22 - f.t) * 2.2), f.col ?? [1, 1, 1], f.t * 2.5, 7, false);
  for (const f of fx) if (f.kind === "tracer" && f.to) {
    const d = sub(f.to, f.pos), L = len(d);
    R.draw(M.tracer, trs(f.pos, Math.atan2(d[0], d[2]), -Math.asin(clamp(d[1] / L, -1, 1)), [1, 1, L]), [1, 1, 1], 1, 0, false);
  }
  ...
  if (P.build) {
    ...
    R.draw(M[`${bt.type}_${P.mat}`], trs(bt.pos, bt.dir * Math.PI / 2), ok ? [0.5, 1.2, 0.6] : [1.4, 0.5, 0.5], 0.45, MAT_STYLE[P.mat], false);
  }
  if (P.state === "bus" || bus.t < bus.dur + 30) {
    const bp = P.state === "bus" ? bus.pos : add(bus.a, scale(sub(bus.b, bus.a), Math.min(1, (bus.t + (P.matchT - bus.t)) / bus.dur)));
    R.draw(M.bus, trs(bp, bus.yaw));
    R.draw(M.balloon, trs(add(bp, [0, 16, 0]), bus.yaw));
  }
  for (const d of drops) if (!d.landed) {
    R.draw(M.chest, trs(d.pos, 0, 0, 1.3));
    R.draw(M.balloon, trs(add(d.pos, [0, 5.5, 0]), 0, 0, 0.32), [0.4, 0.5, 1]);
  }
  for (const pd of pads) {
    R.draw(M.launchpad, trs(pd.pos, pd.t * 2));
  }
  for (const n of nades) {
    if (n.rocket) R.draw(M.rocket, trs(n.pos, Math.atan2(n.vel[0], n.vel[2]), -Math.asin(clamp(n.vel[1] / len(n.vel), -1, 1))));
    else R.draw(M[n.kind ?? "grenade"], trs(n.pos, n.t * 4, n.t * 3));
  }
  ...
  const pose = P.dyingT > 0 ? "idle" : P.emoteT > 0 ? "emote" : P.state === "sky" ? "sky" : P.state === "glide" ? "glide" : P.swim ? "sky" : P.crouch ? "crouch" : P.build || P.editing ? "build" : P.slot >= 0 && it && it.kind !== "ammo" ? "aim" : "pick";
  const held = P.state !== "play" && P.state !== "island" || P.build || P.editing || P.swim || P.emoteT > 0 ? void 0 : it ? it.kind : "pickaxe";
  if (P.dead && P.dyingT > 0) {
    const k = clamp((1.5 - P.dyingT) / 0.5, 0, 1);
    drawChar(CHARS[P.skin], mul(trs(P.pos, P.yaw), mul(translate(0, 0, -0.3 * k), rotX(-1.45 * k * k))), { anim: 0, speed: 0, grounded: true, pitch: 0, pose: "idle" });
  }
  if (P.state !== "bus" && !P.dead) {
    ...
    if ((P.thirdPerson || P.state !== "play") && !P.scoped) {
      ...
      if (it) R.draw(M[it.kind], trs(hp, P.yaw, -P.pitch), [1, 1, 1], 1, 0, false);
      else R.draw(M.pickaxe, mul(trs(hp, P.yaw, -P.pitch), rotX(1 + (P.swing > 0 ? Math.sin(P.swing * 6.3) * 1.2 : 0))), [1, 1, 1], 1, 0, false);
    }
  }
  R.shadows = S.shadows;
  R.scale = S.scale;
  ...
  PROF.flush += pf3 - pf2;
  try {
    drawIcon(SKINS[P.skin]);
    drawHud();
  } catch (e) {
    if (!window._hudErr) {
      window._hudErr = e;
      console.error("drawHud", e);
    }
  }
  PROF.hud += performance.now() - pf3;
  PROF.frames++;
  gpPrev.clear();
...
}
{
  ...
  if (q.get("auto") === "play") setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      const at = (q.get("at") || "40,125").split(",").map(Number);
      P.pos = [at[0], 0, at[1]];
      P.pos[1] = W.groundH(at[0], at[1], 0) + 0.3;
      P.vel = [0, 0, 0];
      P.yaw = +(q.get("yaw") || 0.3);
      P.pitch = +(q.get("pitch") || 0.05);
      P.inv[0] = mkItem("ar", 1, 2);
      P.slot = q.get("slot") === "pick" ? -1 : 0;
      if (q.get("hud") === "0") H.hud.style.display = "none";
      if (q.get("bot")) {
        for (const o of bots) o.dead = true;
        const b = spawnBot([at[0] - 7, P.pos[1] + 1, at[1] - 12], 2);
        b.state = "ground";
        b.yaw = 2.6;
      }
      if (q.get("fly")) {
